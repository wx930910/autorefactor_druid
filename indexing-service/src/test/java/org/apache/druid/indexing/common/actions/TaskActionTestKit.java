...
public class TaskActionTestKit extends ExternalResource {
	private final MetadataStorageTablesConfig metadataStorageTablesConfig = MetadataStorageTablesConfig
			.fromBase("druid");
	...
	public void before() {
		...
		metadataStorageCoordinator = new IndexerSQLMetadataStorageCoordinator(objectMapper, metadataStorageTablesConfig,
				testDerbyConnector);
		taskLockbox = new TaskLockbox(taskStorage, metadataStorageCoordinator);
		segmentsMetadataManager = new SqlSegmentsMetadataManager(objectMapper,
		...
		taskActionToolbox = new TaskActionToolbox(taskLockbox, taskStorage, metadataStorageCoordinator,
				NoopServiceEmitter.mockServiceEmitter1(), EasyMock.createMock(SupervisorManager.class));
