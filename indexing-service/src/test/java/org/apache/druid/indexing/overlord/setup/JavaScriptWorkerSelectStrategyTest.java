...
public class JavaScriptWorkerSelectStrategyTest {
	...
	public void testSerde() throws Exception {
		...
		mapper.setInjectableValues(
				new InjectableValues.Std().addValue(JavaScriptConfig.class, JavaScriptConfig.getEnabledInstance()));

		Assert.assertEquals(STRATEGY,
				mapper.readValue(mapper.writeValueAsString(STRATEGY), WorkerSelectStrategy.class));
	}
	...
	public void testDisabled() throws Exception {
		ObjectMapper mapper = new DefaultObjectMapper();
		mapper.setInjectableValues(
				new InjectableValues.Std().addValue(JavaScriptConfig.class, new JavaScriptConfig(false)));
	...
	public void testFindWorkerForTask() {
		...
		ImmutableWorkerInfo workerForBatchTask = STRATEGY.findWorkerForTask(
				TestRemoteTaskRunnerConfig.mockRemoteTaskRunnerConfig1(new Period("PT1S")), workerMap,
	...
	public void testIsolationOfBatchWorker() {
		ImmutableMap<String, ImmutableWorkerInfo> workerMap = ImmutableMap.of("10.0.0.1",
				createMockWorker(1, true, true), "10.0.0.2", createMockWorker(1, true, true));
		ImmutableWorkerInfo workerForOtherTask = STRATEGY.findWorkerForTask(
				TestRemoteTaskRunnerConfig.mockRemoteTaskRunnerConfig1(new Period("PT1S")), workerMap,
				createMockTask("other_type"));
	...
	public void testNoValidWorker() {
		ImmutableMap<String, ImmutableWorkerInfo> workerMap = ImmutableMap.of("10.0.0.1",
				createMockWorker(1, true, false), "10.0.0.4", createMockWorker(1, true, false));
		ImmutableWorkerInfo workerForBatchTask = STRATEGY.findWorkerForTask(
				TestRemoteTaskRunnerConfig.mockRemoteTaskRunnerConfig1(new Period("PT1S")), workerMap,
				createMockTask("index_hadoop"));
	...
	public void testNoWorkerCanRunTask() {
		ImmutableMap<String, ImmutableWorkerInfo> workerMap = ImmutableMap.of("10.0.0.1",
				createMockWorker(1, false, true), "10.0.0.4", createMockWorker(1, false, true));
		ImmutableWorkerInfo workerForBatchTask = STRATEGY.findWorkerForTask(
				TestRemoteTaskRunnerConfig.mockRemoteTaskRunnerConfig1(new Period("PT1S")), workerMap,
				createMockTask("index_hadoop"));
		Assert.assertNull(workerForBatchTask);
	...
	public void testFillWorkerCapacity() {
		// tasks shoudl be assigned to the worker with maximum currCapacity used until
		// its full
		ImmutableMap<String, ImmutableWorkerInfo> workerMap = ImmutableMap.of("10.0.0.1",
				createMockWorker(1, true, true), "10.0.0.2", createMockWorker(5, true, true));
		ImmutableWorkerInfo workerForBatchTask = STRATEGY.findWorkerForTask(
				TestRemoteTaskRunnerConfig.mockRemoteTaskRunnerConfig1(new Period("PT1S")), workerMap,
				createMockTask("index_hadoop"));
