...
public class SeekableStreamSupervisorStateTest extends EasyMockSupport {
	...
	public void setupTest() {
		...
		emitter = Mockito.spy(new ServiceEmitter("", "", null));
		try {
		...
		EasyMock.expect(taskClientFactory.build(EasyMock.anyObject(), EasyMock.anyString(), EasyMock.anyInt(),
				EasyMock.anyObject(), EasyMock.anyLong())).andReturn(indexTaskClient).anyTimes();
	...
	public void testRunningStreamGetSequenceNumberReturnsNull() throws Exception {
		...
		Assert.assertEquals(
				StringUtils.format("unable to fetch sequence number for partition[%s] from stream", SHARD_ID),
		...
		Assert.assertEquals(BasicState.UNHEALTHY_SUPERVISOR,
	...
	public void testConnectingToStreamFail() throws Exception {
		...
		EasyMock.expect(recordSupplier.getPartitionIds(STREAM))
				.andThrow(new StreamException(new IllegalStateException(EXCEPTION_MSG))).anyTimes();
		EasyMock.expect(taskStorage.getActiveTasksByDatasource(DATASOURCE)).andReturn(ImmutableList.of()).anyTimes();
		EasyMock.expect(taskQueue.add(EasyMock.anyObject())).andReturn(true).anyTimes();
		...
		Assert.assertEquals(StringUtils.format("%s: %s", IllegalStateException.class.getName(), EXCEPTION_MSG),
				exceptionEvents.get(0).getMessage());
		Assert.assertFalse(supervisor.stateManager.isAtLeastOneSuccessfulRun());
		...
		Assert.assertFalse(supervisor.stateManager.isHealthy());
		Assert.assertEquals(SeekableStreamState.UNABLE_TO_CONNECT_TO_STREAM,
	...
	public void testConnectingToStreamFailRecoveryFailRecovery() throws Exception {
		EasyMock.expect(spec.isSuspended()).andReturn(false).anyTimes();
		EasyMock.expect(recordSupplier.getPartitionIds(STREAM))
				.andThrow(new StreamException(new IllegalStateException())).times(3);
	...
	public void testDiscoveringInitialTasksFailRecoveryFail() throws Exception {
		...
		EasyMock.expect(taskStorage.getActiveTasksByDatasource(DATASOURCE))
				.andThrow(new IllegalStateException(EXCEPTION_MSG)).times(3);
	...
	public void testCreatingTasksFailRecoveryFail() throws Exception {
		...
		EasyMock.expect(recordSupplier.getPartitionIds(STREAM)).andReturn(ImmutableSet.of(SHARD_ID)).anyTimes();
		EasyMock.expect(taskStorage.getActiveTasksByDatasource(DATASOURCE)).andReturn(ImmutableList.of()).anyTimes();
		EasyMock.expect(taskQueue.add(EasyMock.anyObject())).andThrow(new IllegalStateException(EXCEPTION_MSG))
				.times(3);
	...
	public void testEmitBothLag() throws Exception {
		...
		TestEmittingTestSeekableStreamSupervisor supervisor = new TestEmittingTestSeekableStreamSupervisor(latch,
		...
		latch.await();
		Assert.assertEquals(6, emitterEvents.size());
		Assert.assertEquals("ingest/test/lag", emitterEvents.get(0).toMap().get("metric"));
		Assert.assertEquals(850L, emitterEvents.get(0).toMap().get("value"));
		Assert.assertEquals("ingest/test/maxLag", emitterEvents.get(1).toMap().get("metric"));
		Assert.assertEquals(500L, emitterEvents.get(1).toMap().get("value"));
		Assert.assertEquals("ingest/test/avgLag", emitterEvents.get(2).toMap().get("metric"));
		Assert.assertEquals(283L, emitterEvents.get(2).toMap().get("value"));
		Assert.assertEquals("ingest/test/lag/time", emitterEvents.get(3).toMap().get("metric"));
		Assert.assertEquals(45000L, emitterEvents.get(3).toMap().get("value"));
		Assert.assertEquals("ingest/test/maxLag/time", emitterEvents.get(4).toMap().get("metric"));
		Assert.assertEquals(20000L, emitterEvents.get(4).toMap().get("value"));
		Assert.assertEquals("ingest/test/avgLag/time", emitterEvents.get(5).toMap().get("metric"));
		Assert.assertEquals(15000L, emitterEvents.get(5).toMap().get("value"));
		verifyAll();
	}
	...
	public void testEmitRecordLag() throws Exception {
		...
		CountDownLatch latch = new CountDownLatch(1);
		TestEmittingTestSeekableStreamSupervisor supervisor = new TestEmittingTestSeekableStreamSupervisor(latch,
				ImmutableMap.of("1", 100L, "2", 250L, "3", 500L), null);
		...
		latch.await();
		Assert.assertEquals(3, emitterEvents.size());
		Assert.assertEquals("ingest/test/lag", emitterEvents.get(0).toMap().get("metric"));
		Assert.assertEquals(850L, emitterEvents.get(0).toMap().get("value"));
		Assert.assertEquals("ingest/test/maxLag", emitterEvents.get(1).toMap().get("metric"));
		Assert.assertEquals(500L, emitterEvents.get(1).toMap().get("value"));
	...
	public void testEmitTimeLag() throws Exception {
		...
		CountDownLatch latch = new CountDownLatch(1);
		TestEmittingTestSeekableStreamSupervisor supervisor = new TestEmittingTestSeekableStreamSupervisor(latch, null,
				ImmutableMap.of("1", 10000L, "2", 15000L, "3", 20000L));
		...
		latch.await();
		Assert.assertEquals(3, emitterEvents.size());
		Assert.assertEquals("ingest/test/lag/time", emitterEvents.get(0).toMap().get("metric"));
		Assert.assertEquals(45000L, emitterEvents.get(0).toMap().get("value"));
		Assert.assertEquals("ingest/test/maxLag/time", emitterEvents.get(1).toMap().get("metric"));
		Assert.assertEquals(20000L, emitterEvents.get(1).toMap().get("value"));
		Assert.assertEquals("ingest/test/avgLag/time", emitterEvents.get(2).toMap().get("metric"));
		Assert.assertEquals(15000L, emitterEvents.get(2).toMap().get("value"));
		verifyAll();
	}
	...
	public void testEmitNoLagWhenSuspended() throws Exception {
		...
		latch.await();
		Assert.assertEquals(0, emitterEvents.size());
		verifyAll();
	}
	...
	private void expectEmitterSupervisor(boolean suspended) throws EntryExistsException {
		...
		EasyMock.expect(spec.getIoConfig())
				.andReturn(
						new SeekableStreamSupervisorIOConfig("stream",
								new JsonInputFormat(new JSONPathSpec(true, ImmutableList.of()), ImmutableMap.of(),
										false),
								1, 1, new Period("PT1H"), new Period("PT1S"), new Period("PT30S"), false,
								new Period("PT30M"), null, null, null) {
						})
				.anyTimes();
		EasyMock.expect(spec.getTuningConfig()).andReturn(getTuningConfig()).anyTimes();
		EasyMock.expect(spec.getEmitter()).andReturn(emitter).anyTimes();
	...
	private static SeekableStreamSupervisorIOConfig getIOConfig() {
		return new SeekableStreamSupervisorIOConfig("stream",
				new JsonInputFormat(new JSONPathSpec(true, ImmutableList.of()), ImmutableMap.of(), false), 1, 1,
				new Period("PT1H"), new Period("P1D"), new Period("PT30S"), false, new Period("PT30M"), null, null,
				null) {
		};
	}
	...
	private static SeekableStreamSupervisorTuningConfig getTuningConfig() {
		return new SeekableStreamSupervisorTuningConfig() {
			...
			public SeekableStreamIndexTaskTuningConfig convertToTaskTuningConfig() {
				return new SeekableStreamIndexTaskTuningConfig(null, null, null, null, null, null, null, null, null,
	...
	private class TestSeekableStreamIndexTask extends SeekableStreamIndexTask<String, String> {
		public TestSeekableStreamIndexTask(String id, @Nullable TaskResource taskResource, DataSchema dataSchema,
				SeekableStreamIndexTaskTuningConfig tuningConfig,
				SeekableStreamIndexTaskIOConfig<String, String> ioConfig, @Nullable Map<String, Object> context,
				@Nullable ChatHandlerProvider chatHandlerProvider, AuthorizerMapper authorizerMapper,
				RowIngestionMetersFactory rowIngestionMetersFactory, @Nullable String groupId) {
			super(id, taskResource, dataSchema, tuningConfig, ioConfig, context, chatHandlerProvider, authorizerMapper,
					rowIngestionMetersFactory, groupId, null);
		}
	...
	private abstract class BaseTestSeekableStreamSupervisor extends SeekableStreamSupervisor<String, String> {
		private BaseTestSeekableStreamSupervisor() {
			super("testSupervisorId", taskStorage, taskMaster, indexerMetadataStorageCoordinator, taskClientFactory,
					OBJECT_MAPPER, spec, rowIngestionMetersFactory, false);
		}
		...
		protected SeekableStreamIndexTaskIOConfig createTaskIoConfig(int groupId, Map<String, String> startPartitions,
			...
			return new SeekableStreamIndexTaskIOConfig<String, String>(groupId, baseSequenceName,
					new SeekableStreamStartSequenceNumbers<>(STREAM, startPartitions,
							exclusiveStartSequenceNumberPartitions),
					new SeekableStreamEndSequenceNumbers<>(STREAM, endPartitions), true, minimumMessageTime,
					maximumMessageTime, ioConfig.getInputFormat()) {
			};
		}
		...
		protected List<SeekableStreamIndexTask<String, String>> createIndexTasks(int replicas, String baseSequenceName,
			...
			return ImmutableList.of(new TestSeekableStreamIndexTask("id", null, getDataSchema(), taskTuningConfig,
					taskIoConfig, null, null, null, rowIngestionMetersFactory, null));
		}
		...
		protected SeekableStreamSupervisorReportPayload<String, String> createReportPayload(int numPartitions,
			...
			return new SeekableStreamSupervisorReportPayload<String, String>(DATASOURCE, STREAM, 1, 1, 1L, null, null,
					null, null, null, null, false, true, null, null, null) {
			};
		}
	...
	private class TestEmittingTestSeekableStreamSupervisor extends BaseTestSeekableStreamSupervisor {
		...
		TestEmittingTestSeekableStreamSupervisor(CountDownLatch latch, Map<String, Long> partitionsRecordLag,
		...
		protected void scheduleReporting(ScheduledExecutorService reportingExec) {
			...
			reportingExec.scheduleAtFixedRate(this::emitLag, ioConfig.getStartDelay().getMillis(),
					spec.getMonitorSchedulerConfig().getEmitterPeriod().getMillis(), TimeUnit.MILLISECONDS);
		}
	}
}
