...
public class HttpRemoteTaskRunnerTest {
	static public DruidNodeDiscovery mockDruidNodeDiscovery1() {
		Listener[] mockFieldVariableListener = new Listener[1];
		DruidNodeDiscovery mockInstance = Mockito.spy(DruidNodeDiscovery.class);
		try {
			Mockito.doAnswer((stubInvo) -> {
				throw new UnsupportedOperationException("Not Implemented.");
			}).when(mockInstance).getAllNodes();
			Mockito.doAnswer((stubInvo) -> {
				Listener listener = stubInvo.getArgument(0);
				listener.nodesAdded(ImmutableList.of());
				listener.nodeViewInitialized();
				mockFieldVariableListener[0] = listener;
				return null;
			}).when(mockInstance).registerListener(Mockito.any(Listener.class));
	...
	/*
	 * Simulates startup of Overlord and Workers being discovered with no previously
	 * known tasks. Fresh tasks are given and expected to be completed.
	 */
	@Test(timeout = 60_000L)
	public void testFreshStart() throws Exception {
		DruidNodeDiscovery druidNodeDiscovery = Mockito.spy(DruidNodeDiscovery.class);
		Listener[] druidNodeDiscoveryListener = new Listener[1];
		try {
			Mockito.doAnswer((stubInvo) -> {
				throw new UnsupportedOperationException("Not Implemented.");
			}).when(druidNodeDiscovery).getAllNodes();
			Mockito.doAnswer((stubInvo) -> {
				Listener listener = stubInvo.getArgument(0);
				listener.nodesAdded(ImmutableList.of());
				listener.nodeViewInitialized();
				druidNodeDiscoveryListener[0] = listener;
				return null;
			}).when(druidNodeDiscovery).registerListener(Mockito.any(Listener.class));
		} catch (Exception exception) {
		}
		...
		HttpRemoteTaskRunner taskRunner = new HttpRemoteTaskRunner(TestHelper.makeJsonMapper(),
				...
				new NoopProvisioningStrategy<>(), druidNodeDiscoveryProvider,
				EasyMock.createNiceMock(TaskStorage.class), EasyMock.createNiceMock(CuratorFramework.class),
				new IndexerZkConfig(new ZkPathsConfig(), null, null, null, null)) {
			@Override
			protected WorkerHolder createWorkerHolder(ObjectMapper smileMapper, HttpClient httpClient,
					HttpRemoteTaskRunnerConfig config, ScheduledExecutorService workersSyncExec,
					WorkerHolder.Listener listener, Worker worker, List<TaskAnnouncement> knownAnnouncements) {
				return HttpRemoteTaskRunnerTest.createWorkerHolder(smileMapper, httpClient, config, workersSyncExec,
						listener, worker, ImmutableList.of(), ImmutableList.of(), ImmutableMap.of(),
						new AtomicInteger(), ImmutableSet.of());
			}
		};
		...
		DiscoveryDruidNode druidNode1 = new DiscoveryDruidNode(
				new DruidNode("service", "host1", false, 8080, null, true, false), NodeRole.MIDDLE_MANAGER,
				ImmutableMap.of(WorkerNodeService.DISCOVERY_SERVICE_KEY,
						new WorkerNodeService("ip1", 2, "0", WorkerConfig.DEFAULT_CATEGORY)));

		DiscoveryDruidNode druidNode2 = new DiscoveryDruidNode(
				new DruidNode("service", "host2", false, 8080, null, true, false), NodeRole.MIDDLE_MANAGER,
				ImmutableMap.of(WorkerNodeService.DISCOVERY_SERVICE_KEY,
						new WorkerNodeService("ip2", 2, "0", WorkerConfig.DEFAULT_CATEGORY)));

		druidNodeDiscoveryListener[0].nodesAdded(ImmutableList.of(druidNode1, druidNode2));
	 ...
	 * to a worker. But, other tasks are successfully assigned to other worker and
	 * get completed.
	 */
	@Test(timeout = 60_000L)
	public void testOneStuckTaskAssignmentDoesntBlockOthers() throws Exception {
		...
		HttpRemoteTaskRunner taskRunner = new HttpRemoteTaskRunner(TestHelper.makeJsonMapper(),
			...
			protected WorkerHolder createWorkerHolder(ObjectMapper smileMapper, HttpClient httpClient,
					...
					WorkerHolder.Listener listener, Worker worker, List<TaskAnnouncement> knownAnnouncements) {
				return HttpRemoteTaskRunnerTest.createWorkerHolder(smileMapper, httpClient, config, workersSyncExec,
						listener, worker, ImmutableList.of(), ImmutableList.of(),
	...
	public void testTaskRunnerRestart() throws Exception {
		...
		EasyMock.expect(taskStorageMock.getStatus(task3.getId()))
				.andReturn(Optional.of(TaskStatus.running(task3.getId())));
		EasyMock.expect(taskStorageMock.getStatus(task4.getId()))
				.andReturn(Optional.of(TaskStatus.running(task4.getId())));
		EasyMock.expect(taskStorageMock.getStatus(task5.getId()))
				.andReturn(Optional.of(TaskStatus.success(task5.getId())));
		...
		HttpRemoteTaskRunner taskRunner = new HttpRemoteTaskRunner(TestHelper.makeJsonMapper(),
				...
				DSuppliers.of(new AtomicReference<>(DefaultWorkerBehaviorConfig.defaultConfig())),
				new NoopProvisioningStrategy<>(), druidNodeDiscoveryProvider, taskStorageMock,
			...
			protected WorkerHolder createWorkerHolder(ObjectMapper smileMapper, HttpClient httpClient,
				...
				if (workerHolders.containsKey(worker.getHost())) {
					return workerHolders.get(worker.getHost()).apply(smileMapper, httpClient, config, workersSyncExec,
		...
		DiscoveryDruidNode druidNode = new DiscoveryDruidNode(
				new DruidNode("service", "host", false, 1234, null, true, false), NodeRole.MIDDLE_MANAGER,
				ImmutableMap.of(WorkerNodeService.DISCOVERY_SERVICE_KEY,
						new WorkerNodeService("ip1", 2, "0", WorkerConfig.DEFAULT_CATEGORY)));
		...
		workerHolders.put("host:1234",
				(mapper, httpClient, config, exec, listener, worker, knownAnnouncements) -> createWorkerHolder(mapper,
						httpClient, config, exec, listener, worker, knownAnnouncements,
						ImmutableList.of(
								TaskAnnouncement.create(task1, TaskStatus.success(task1.getId()),
										TaskLocation.create("host", 1234, 1235)),
								TaskAnnouncement.create(task2, TaskStatus.running(task2.getId()),
										TaskLocation.create("host", 1234, 1235)),
								TaskAnnouncement.create(task2, TaskStatus.success(task2.getId()),
										TaskLocation.create("host", 1234, 1235)),
								TaskAnnouncement.create(task3, TaskStatus.success(task3.getId()),
										TaskLocation.create("host", 1234, 1235)),
								TaskAnnouncement.create(task4, TaskStatus.running(task4.getId()),
										TaskLocation.create("host", 1234, 1235)),
								TaskAnnouncement.create(task5, TaskStatus.running(task5.getId()),
										TaskLocation.create("host", 1234, 1235))),
						ImmutableMap.of(), ticks, taskShutdowns));

		druidNodeDiscoveryListener[0].nodesAdded(ImmutableList.of(druidNode));
	...
	public void testWorkerDisapperAndReappearBeforeItsCleanup() throws Exception {
		...
		workerHolders.put("host:1234",
				(mapper, httpClient, config, exec, listener, worker, knownAnnouncements) -> createWorkerHolder(mapper,
						httpClient, config, exec, listener, worker, knownAnnouncements, ImmutableList.of(),
						ImmutableMap.of(task1,
								ImmutableList.of(
										TaskAnnouncement.create(task1, TaskStatus.running(task1.getId()),
		...
		druidNodeDiscoveryListener[0].nodesRemoved(ImmutableList.of(druidNode));
		...
		workerHolders.put("host:1234",
				(mapper, httpClient, config, exec, listener, worker, knownAnnouncements) -> createWorkerHolder(mapper,
						...
						), ImmutableMap.of(), new AtomicInteger(), ImmutableSet.of()));
		...
		workerHolders.put("host:1234",
				(mapper, httpClient, config, exec, listener, worker, knownAnnouncements) -> createWorkerHolder(mapper,
						...
						ImmutableList.of(
								...
								TaskAnnouncement.create(task2, TaskStatus.running(task2.getId()),
										TaskLocation.create("host", 1234, 1235))),
						ImmutableMap.of(), ticks, actualShutdowns));
	...
	public void testMarkWorkersLazy() throws Exception {
		...
		workerHolders.put("host1:8080",
				(mapper, httpClient, config, exec, listener, worker, knownAnnouncements) -> createWorkerHolder(mapper,
						...
						ticks, ImmutableSet.of()));

		druidNodeDiscoveryListener[0].nodesAdded(ImmutableList.of(druidNode1));
		...
		workerHolders.put("host2:8080",
				(mapper, httpClient, config, exec, listener, worker, knownAnnouncements) -> createWorkerHolder(mapper,
						httpClient, config, exec, listener, worker, knownAnnouncements, ImmutableList.of(),
						ImmutableMap.of(task2, ImmutableList.of()), ticks, ImmutableSet.of()));

		druidNodeDiscoveryListener[0].nodesAdded(ImmutableList.of(druidNode2));
		...
		DiscoveryDruidNode druidNode3 = new DiscoveryDruidNode(
				new DruidNode("service", "host3", false, 8080, null, true, false), NodeRole.MIDDLE_MANAGER,
				ImmutableMap.of(WorkerNodeService.DISCOVERY_SERVICE_KEY,
						new WorkerNodeService("ip2", 1, "0", WorkerConfig.DEFAULT_CATEGORY)));

		workerHolders.put("host3:8080",
				(mapper, httpClient, config, exec, listener, worker, knownAnnouncements) -> createWorkerHolder(mapper,
						httpClient, config, exec, listener, worker, knownAnnouncements, ImmutableList.of(),
						ImmutableMap.of(), new AtomicInteger(), ImmutableSet.of()));

		druidNodeDiscoveryListener[0].nodesAdded(ImmutableList.of(druidNode3));
		...
		Assert.assertEquals("host3:8080", Iterables
				.getOnlyElement(taskRunner.markWorkersLazy(Predicates.alwaysTrue(), Integer.MAX_VALUE)).getHost());
	}

	/*
	 * Task goes PENDING -> RUNNING -> SUCCESS and few more useless notifications in
	 * between.
	 */
	@Test
	public void testTaskAddedOrUpdated1() throws Exception {
		...
		HttpRemoteTaskRunner taskRunner = createTaskRunnerForTestTaskAddedOrUpdated(
				EasyMock.createStrictMock(TaskStorage.class), listenerNotificationsAccumulator);
		...
		taskRunner.taskAddedOrUpdated(TaskAnnouncement.create(task, TaskStatus.running(task.getId()),
				TaskLocation.create("worker", 1000, 1001)), workerHolder);
		...
		taskRunner.taskAddedOrUpdated(
				TaskAnnouncement.create(task, TaskStatus.running(task.getId()), TaskLocation.create("worker", 1, 2)),
				workerHolder);
		Assert.assertEquals(task.getId(), Iterables.getOnlyElement(taskRunner.getRunningTasks()).getTaskId());
		...
		// Another "rogue-worker" reports running it, and gets asked to shutdown the
		// task
		...
		taskRunner.taskAddedOrUpdated(TaskAnnouncement.create(task, TaskStatus.running(task.getId()),
				TaskLocation.create("rogue-worker", 1, 2)), rogueWorkerHolder);
		Assert.assertEquals(task.getId(), Iterables.getOnlyElement(taskRunner.getRunningTasks()).getTaskId());
		...
		EasyMock.expect(rogueWorkerHolder.getWorker())
				...
				.anyTimes();
		EasyMock.replay(rogueWorkerHolder);
		taskRunner.taskAddedOrUpdated(TaskAnnouncement.create(task, TaskStatus.failure(task.getId()),
				TaskLocation.create("rogue-worker", 1, 2)), rogueWorkerHolder);
		Assert.assertEquals(task.getId(), Iterables.getOnlyElement(taskRunner.getRunningTasks()).getTaskId());
		EasyMock.verify(rogueWorkerHolder);
		...
		taskRunner.taskAddedOrUpdated(
				TaskAnnouncement.create(task, TaskStatus.success(task.getId()), TaskLocation.create("worker", 1, 2)),
				workerHolder);
		...
		Assert.assertEquals(listenerNotificationsAccumulator,
				ImmutableList.of(ImmutableList.of(task.getId(), TaskLocation.create("worker", 1000, 1001)),
						ImmutableList.of(task.getId(), TaskLocation.create("worker", 1, 2)),
						ImmutableList.of(task.getId(), TaskStatus.success(task.getId()))));
	}

	/*
	 * Task goes from PENDING -> SUCCESS . Happens when TaskRunner is given task but
	 * a worker reported it being already completed with SUCCESS.
	 */
	@Test
	public void testTaskAddedOrUpdated2() throws Exception {
		...
		Assert.assertEquals(listenerNotificationsAccumulator,
				ImmutableList.of(ImmutableList.of(task.getId(), TaskLocation.create("worker", 1, 2)),
						ImmutableList.of(task.getId(), TaskStatus.success(task.getId()))));
	}

	/*
	 * Notifications received for tasks not known to TaskRunner maybe known to
	 * TaskStorage. This could happen when TaskRunner starts and workers reports
	...
	public void testTaskAddedOrUpdated3() {
		...
		List<Object> listenerNotificationsAccumulator = new ArrayList<>();
		HttpRemoteTaskRunner taskRunner = createTaskRunnerForTestTaskAddedOrUpdated(taskStorage,
		...
		taskRunner.taskAddedOrUpdated(
				TaskAnnouncement.create(task1, TaskStatus.running(task1.getId()), TaskLocation.create("worker", 1, 2)),
				workerHolder);

		taskRunner.taskAddedOrUpdated(
				TaskAnnouncement.create(task2, TaskStatus.success(task2.getId()), TaskLocation.create("worker", 3, 4)),
				workerHolder);

		taskRunner.taskAddedOrUpdated(
				TaskAnnouncement.create(task3, TaskStatus.running(task3.getId()), TaskLocation.create("worker", 5, 6)),
				workerHolder);

		taskRunner.taskAddedOrUpdated(
				TaskAnnouncement.create(task4, TaskStatus.success(task4.getId()), TaskLocation.create("worker", 7, 8)),
				workerHolder);

		taskRunner.taskAddedOrUpdated(
				TaskAnnouncement.create(task5, TaskStatus.running(task5.getId()), TaskLocation.create("worker", 9, 10)),
				workerHolder);

		taskRunner.taskAddedOrUpdated(TaskAnnouncement.create(task6, TaskStatus.success(task6.getId()),
				TaskLocation.create("worker", 11, 12)), workerHolder);

		EasyMock.verify(workerHolder, taskStorage);

		Assert.assertEquals(listenerNotificationsAccumulator,
				ImmutableList.of(ImmutableList.of(task1.getId(), TaskLocation.create("worker", 1, 2)),
						ImmutableList.of(task2.getId(), TaskLocation.create("worker", 3, 4)),
						ImmutableList.of(task2.getId(), TaskStatus.success(task2.getId()))));
	}

	private HttpRemoteTaskRunner createTaskRunnerForTestTaskAddedOrUpdated(TaskStorage taskStorage,
			List<Object> listenerNotificationsAccumulator) {
		DruidNodeDiscovery druidNodeDiscovery = HttpRemoteTaskRunnerTest.mockDruidNodeDiscovery1();
		DruidNodeDiscoveryProvider druidNodeDiscoveryProvider = EasyMock.createMock(DruidNodeDiscoveryProvider.class);
		EasyMock.expect(druidNodeDiscoveryProvider.getForService(WorkerNodeService.DISCOVERY_SERVICE_KEY))
				.andReturn(druidNodeDiscovery);
		EasyMock.replay(druidNodeDiscoveryProvider);
		...
		HttpRemoteTaskRunner taskRunner = new HttpRemoteTaskRunner(TestHelper.makeJsonMapper(),
				...
				DSuppliers.of(new AtomicReference<>(DefaultWorkerBehaviorConfig.defaultConfig())),
				new NoopProvisioningStrategy<>(), druidNodeDiscoveryProvider, taskStorage,
				EasyMock.createNiceMock(CuratorFramework.class),
				new IndexerZkConfig(new ZkPathsConfig(), null, null, null, null));
	...
	private static WorkerHolder createWorkerHolder(ObjectMapper smileMapper, HttpClient httpClient,
			HttpRemoteTaskRunnerConfig config, ScheduledExecutorService workersSyncExec, WorkerHolder.Listener listener,
			Worker worker, List<TaskAnnouncement> knownAnnouncements,

			// simulates task announcements received from worker on first sync call for the
			...
			// incremented on each runnable completion in workersSyncExec, useful for
			// deterministically watching that some
			// work completed
			AtomicInteger ticks,

			// Updated each time a shutdown(taskId) call is received, useful for asserting
			// that expected shutdowns indeed
			// happened.
		...
		return new WorkerHolder(smileMapper, httpClient, config, workersSyncExec, listener, worker,
			...
			public boolean assignTask(Task task) {
				...
				if (toBeAssignedTasks.containsKey(task)) {
					...
					// no behavior specified for the task, so do default behavior of completing the
					// task
					announcements = new ArrayList<>();
					announcements.add(
							TaskAnnouncement.create(task, TaskStatus.running(task.getId()), TaskLocation.unknown()));
					announcements.add(TaskAnnouncement.create(task, TaskStatus.running(task.getId()),
							TaskLocation.create(workerHost, workerPort, -1)));
					announcements.add(TaskAnnouncement.create(task, TaskStatus.success(task.getId()),
							TaskLocation.create(workerHost, workerPort, -1)));
				}
	...
	private interface CustomFunction {
		WorkerHolder apply(ObjectMapper smileMapper, HttpClient httpClient, HttpRemoteTaskRunnerConfig config,
				ScheduledExecutorService workersSyncExec, WorkerHolder.Listener listener, Worker worker,
