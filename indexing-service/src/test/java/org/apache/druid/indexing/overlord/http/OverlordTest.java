...
public class OverlordTest {
	...
	static public TaskRunner mockTaskRunner1(CountDownLatch[] runLatches, CountDownLatch[] completionLatches) {
		List<String>[] mockFieldVariableRunningTasks = new List[1];
		ConcurrentHashMap<String, TaskRunnerWorkItem>[] mockFieldVariableTaskRunnerWorkItems = new ConcurrentHashMap[1];
		CountDownLatch[][] mockFieldVariableCompletionLatches = new CountDownLatch[1][];
		CountDownLatch[][] mockFieldVariableRunLatches = new CountDownLatch[1][];
		TaskRunner mockInstance = Mockito.spy(TaskRunner.class);
		mockFieldVariableRunLatches[0] = runLatches;
		mockFieldVariableCompletionLatches[0] = completionLatches;
		mockFieldVariableTaskRunnerWorkItems[0] = new ConcurrentHashMap<>();
		mockFieldVariableRunningTasks[0] = new ArrayList<>();
		try {
			Mockito.doAnswer((stubInvo) -> {
				return mockFieldVariableTaskRunnerWorkItems[0].values();
			}).when(mockInstance).getKnownTasks();
			Mockito.doAnswer((stubInvo) -> {
				return ImmutableList.of();
			}).when(mockInstance).getPendingTasks();
			Mockito.doAnswer((stubInvo) -> {
				return Optional.absent();
			}).when(mockInstance).getScalingStats();
			Mockito.doAnswer((stubInvo) -> {
				throw new UnsupportedOperationException();
			}).when(mockInstance).unregisterListener(Mockito.any(String.class));
			Mockito.doAnswer((stubInvo) -> {
				Task task = stubInvo.getArgument(0);
				final String taskId = task.getId();
				ListenableFuture<TaskStatus> future = MoreExecutors
						.listeningDecorator(Execs.singleThreaded("noop_test_task_exec_%s"))
						.submit(new Callable<TaskStatus>() {
							@Override
							public TaskStatus call() throws Exception {
								mockFieldVariableRunningTasks[0].add(taskId);
								if (mockFieldVariableRunLatches[0] != null) {
									mockFieldVariableRunLatches[0][Integer.parseInt(taskId)].countDown();
								}
								if (mockFieldVariableCompletionLatches[0] != null) {
									mockFieldVariableCompletionLatches[0][Integer.parseInt(taskId)].await();
								}
								mockFieldVariableTaskRunnerWorkItems[0].remove(taskId);
								mockFieldVariableRunningTasks[0].remove(taskId);
				...
				mockFieldVariableTaskRunnerWorkItems[0].put(taskId, taskRunnerWorkItem);
				return future;
			}).when(mockInstance).run(Mockito.any(Task.class));
			Mockito.doAnswer((stubInvo) -> {
				return Lists.transform(mockFieldVariableRunningTasks[0], new Function<String, TaskRunnerWorkItem>() {
					@Nullable
					@Override
					public TaskRunnerWorkItem apply(String input) {
						return mockFieldVariableTaskRunnerWorkItems[0].get(input);
					}
				});
			}).when(mockInstance).getRunningTasks();
			Mockito.doAnswer((stubInvo) -> {
				throw new UnsupportedOperationException();
			}).when(mockInstance).registerListener(Mockito.any(TaskRunnerListener.class), Mockito.any(Executor.class));
			Mockito.doAnswer((stubInvo) -> {
				return ImmutableList.of();
			}).when(mockInstance).restore();
		} catch (Exception exception) {
		}
		return mockInstance;
	}
	...
	private void setupServerAndCurator() throws Exception {
		...
		curator = CuratorFrameworkFactory.builder().connectString(server.getConnectString())
				...
				.retryPolicy(new RetryOneTime(1)).compressionProvider(new PotentiallyGzippedCompressionProvider(true))
				.build();
	}
	...
	public void setUp() throws Exception {
		...
		EasyMock.expect(req.getAttribute(AuthConfig.DRUID_AUTHENTICATION_RESULT))
				.andReturn(new AuthenticationResult("druid", "druid", null, null)).anyTimes();
		...
		ServiceEmitter serviceEmitter = NoopServiceEmitter.mockServiceEmitter1();
		taskMaster = new TaskMaster(new TaskLockConfig(),
				new TaskQueueConfig(null, new Period(1), null, new Period(10)), taskLockbox, taskStorage,
				taskActionClientFactory, druidNode, new TaskRunnerFactory<TaskRunner>() {
					@Override
					public TaskRunner build() {
						return OverlordTest.mockTaskRunner1(runTaskCountDownLatches, taskCompletionCountDownLatches);
					}
				...
				}, new CoordinatorOverlordServiceConfig(null, null), serviceEmitter, supervisorManager,
				EasyMock.createNiceMock(OverlordHelperManager.class), OverlordTest.mockDruidLeaderSelector1());
	...
	public void testOverlordRun() throws Exception {
		...
		while (!taskMaster.isLeader()) {
			// I believe the control will never reach here and thread will never sleep but
		...
		final WorkerTaskRunnerQueryAdapter workerTaskRunnerQueryAdapter = new WorkerTaskRunnerQueryAdapter(taskMaster,
				null);
		// Test Overlord resource stuff
		overlordResource = new OverlordResource(taskMaster, taskStorageQueryAdapter,
				new IndexerMetadataStorageAdapter(taskStorageQueryAdapter, null), null, null, null,
				AuthTestUtils.TEST_AUTHORIZER_MAPPER, workerTaskRunnerQueryAdapter);
		...
		Assert.assertEquals(TaskStatus.running(taskId_0).getStatusCode(),
	...
	/*
	 * Wait until the task with given taskId has the given Task Status These method
	 * will not timeout until the condition is met so calling method should ensure
	 * timeout This method also assumes that the task with given taskId is present
	 */
