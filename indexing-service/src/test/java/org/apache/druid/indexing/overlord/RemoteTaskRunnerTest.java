...
public class RemoteTaskRunnerTest {
	...
	private static final String ANNOUCEMENTS_PATH = JOINER.join(RemoteTaskRunnerTestUtils.ANNOUNCEMENTS_PATH,
			WORKER_HOST);
	...
	public void testStartWithNoWorker() {
		makeRemoteTaskRunner(TestRemoteTaskRunnerConfig.mockRemoteTaskRunnerConfig1(new Period("PT1S")));
	}
	...
	public void testRunSameAvailabilityGroup() throws Exception {
		...
		TestRealtimeTask task1 = new TestRealtimeTask("rt1", new TaskResource("rt1", 1), "foo",
				TaskStatus.running("rt1"), jsonMapper);
		remoteTaskRunner.run(task1);
		Assert.assertTrue(taskAnnounced(task1.getId()));
		mockWorkerRunningTask(task1);

		TestRealtimeTask task2 = new TestRealtimeTask("rt2", new TaskResource("rt1", 1), "foo",
				TaskStatus.running("rt2"), jsonMapper);
		remoteTaskRunner.run(task2);

		TestRealtimeTask task3 = new TestRealtimeTask("rt3", new TaskResource("rt2", 1), "foo",
				TaskStatus.running("rt3"), jsonMapper);
	...
	public void testRunWithCapacity() throws Exception {
		...
		TestRealtimeTask task2 = new TestRealtimeTask("rt2", new TaskResource("rt2", 3), "foo",
				TaskStatus.running("rt2"), jsonMapper);
		remoteTaskRunner.run(task2);

		TestRealtimeTask task3 = new TestRealtimeTask("rt3", new TaskResource("rt3", 2), "foo",
				TaskStatus.running("rt3"), jsonMapper);
		remoteTaskRunner.run(task3);
	...
	public void testBootstrap() throws Exception {
		...
		RemoteTaskRunnerConfig rtrConfig = TestRemoteTaskRunnerConfig.mockRemoteTaskRunnerConfig1(timeoutPeriod);
		rtrConfig.setMaxPercentageBlacklistWorkers(100);

		makeRemoteTaskRunner(rtrConfig);

		TestRealtimeTask task1 = new TestRealtimeTask("first", new TaskResource("first", 1), "foo",
				TaskStatus.running("first"), jsonMapper);
		remoteTaskRunner.run(task1);
		Assert.assertTrue(taskAnnounced(task1.getId()));
		mockWorkerRunningTask(task1);

		TestRealtimeTask task = new TestRealtimeTask("second", new TaskResource("task", 2), "foo",
				TaskStatus.running("task"), jsonMapper);
		remoteTaskRunner.run(task);

		TestRealtimeTask task2 = new TestRealtimeTask("second", new TaskResource("second", 2), "foo",
				TaskStatus.running("second"), jsonMapper);
		remoteTaskRunner.run(task2);
		Assert.assertTrue(taskAnnounced(task2.getId()));
		mockWorkerRunningTask(task2);

		final Set<String> runningTasks = Sets.newHashSet(Iterables.transform(remoteTaskRunner.getRunningTasks(),
	...
	public void testRunWithTaskComplete() throws Exception {
		doSetup();
		TestRealtimeTask task1 = new TestRealtimeTask("testTask", new TaskResource("testTask", 2), "foo",
				TaskStatus.success("testTask"), jsonMapper);
		remoteTaskRunner.run(task1);
		Assert.assertTrue(taskAnnounced(task1.getId()));
		mockWorkerRunningTask(task1);
	...
	public void testWorkerRemoved() throws Exception {
		...
		Assert.assertTrue(TestUtils.conditionValid(new IndexingServiceCondition() {
			...
			public boolean isValid() {
				...
				// cleanup task is independently scheduled by event listener. we need to wait
				// some more time.
	...
	public void testRestartRemoteTaskRunner() throws Exception {
		...
		remoteTaskRunner.stop();
		makeRemoteTaskRunner(TestRemoteTaskRunnerConfig.mockRemoteTaskRunnerConfig1(new Period("PT5S")));
		final RemoteTaskRunnerWorkItem newWorkItem = remoteTaskRunner.getKnownTasks().stream()
				.filter(workItem -> workItem.getTaskId().equals(task.getId())).findFirst().orElse(null);
	...
	public void testWorkerZKReconnect() throws Exception {
		makeWorker();
		makeRemoteTaskRunner(TestRemoteTaskRunnerConfig.mockRemoteTaskRunnerConfig1(new Period("PT5M")));
		Future<TaskStatus> future = remoteTaskRunner.run(task);
	...
	public void testBlacklistZKWorkers() throws Exception {
		...
		TestRealtimeTask task1 = new TestRealtimeTask("realtime1", new TaskResource("realtime1", 1), "foo",
				TaskStatus.success("realtime1"), jsonMapper);
		...
		TestRealtimeTask task2 = new TestRealtimeTask("realtime2", new TaskResource("realtime2", 1), "foo",
				TaskStatus.running("realtime2"), jsonMapper);
		...
		TestRealtimeTask task3 = new TestRealtimeTask("realtime3", new TaskResource("realtime3", 1), "foo",
				TaskStatus.running("realtime3"), jsonMapper);
	...
	/**
	 * With 2 workers and maxPercentageBlacklistWorkers(25), neither worker should
	 * ever be blacklisted even after exceeding maxRetriesBeforeBlacklist.
	 */
	@Test
	public void testBlacklistZKWorkers25Percent() throws Exception {
		...
		for (int i = 1; i < 13; i++) {
			...
			TestRealtimeTask task = new TestRealtimeTask(taskId, new TaskResource(taskId, 1), "foo",
					TaskStatus.success(taskId), jsonMapper);
	...
	/**
	 * With 2 workers and maxPercentageBlacklistWorkers(50), one worker should get
	 * blacklisted after the second failure and the second worker should never be
	 * blacklisted even after exceeding maxRetriesBeforeBlacklist.
	 */
	@Test
	public void testBlacklistZKWorkers50Percent() throws Exception {
		...
		for (int i = 1; i < 13; i++) {
			...
			Assert.assertEquals(i > 4 ? i - 2 : ((i + 1) / 2),
					remoteTaskRunner.findWorkerRunningTask(task.getId()).getContinuouslyFailedTasksCount());
		}
	}
	...
	public void testSuccessfulTaskOnBlacklistedWorker() throws Exception {
		...
		TestRealtimeTask task1 = new TestRealtimeTask("realtime1", new TaskResource("realtime1", 1), "foo",
				TaskStatus.success("realtime1"), jsonMapper);
		TestRealtimeTask task2 = new TestRealtimeTask("realtime2", new TaskResource("realtime2", 1), "foo",
				TaskStatus.success("realtime2"), jsonMapper);
		TestRealtimeTask task3 = new TestRealtimeTask("realtime3", new TaskResource("realtime3", 1), "foo",
				TaskStatus.success("realtime3"), jsonMapper);
	...
	public void testStatusListenerEventDataNullShouldNotThrowException() throws Exception {
		// Set up mock emitter to verify log alert when exception is thrown inside the
		// status listener
