...
public class IndexedTableJoinMatcherTest {
	...
	public static class ConditionMatcherFactoryTest {
		public static class MakeDimensionProcessorTest {
			static public ConstantDimensionSelector mockConstantDimensionSelector1(String value, int valueCardinality) {
				int[] mockFieldVariableValueCardinality = new int[1];
				ConstantDimensionSelector mockInstance = Mockito.spy(new ConstantDimensionSelector(value));
				mockFieldVariableValueCardinality[0] = valueCardinality;
				try {
					Mockito.doAnswer((stubInvo) -> {
						return mockFieldVariableValueCardinality[0];
					}).when(mockInstance).getValueCardinality();
			...
			public void testMatchMultiValuedRowCardinalityUnknownShouldThrowException() {
				...
				Mockito.doReturn(DimensionDictionarySelector.CARDINALITY_UNKNOWN).when(dimensionSelector)
						.getValueCardinality();

				IndexedTableJoinMatcher.ConditionMatcherFactory conditionMatcherFactory = new IndexedTableJoinMatcher.ConditionMatcherFactory(
						ValueType.STRING, IndexedTableJoinMatcherTest::createSingletonIntList);
				Supplier<IntIterator> dimensionProcessor = conditionMatcherFactory
						.makeDimensionProcessor(dimensionSelector, false);
			...
			private static Supplier<IntIterator> makeDimensionProcessor(int valueCardinality) {
				...
				return conditionMatcherFactory.makeDimensionProcessor(
						MakeDimensionProcessorTest.mockConstantDimensionSelector1(KEY, valueCardinality), false);
			}
		}
	}
