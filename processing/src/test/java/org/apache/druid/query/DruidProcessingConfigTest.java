...
public class DruidProcessingConfigTest {
	static public RuntimeInfo mockRuntimeInfo1(int availableProcessors, long directSize, long maxHeapSize) {
		long[] mockFieldVariableMaxHeapSize = new long[1];
		long[] mockFieldVariableDirectSize = new long[1];
		int[] mockFieldVariableAvailableProcessors = new int[1];
		RuntimeInfo mockInstance = Mockito.spy(RuntimeInfo.class);
		mockFieldVariableAvailableProcessors[0] = availableProcessors;
		mockFieldVariableDirectSize[0] = directSize;
		mockFieldVariableMaxHeapSize[0] = maxHeapSize;
		try {
			Mockito.doAnswer((stubInvo) -> {
				return mockFieldVariableMaxHeapSize[0];
			}).when(mockInstance).getMaxHeapSizeBytes();
			Mockito.doAnswer((stubInvo) -> {
				return mockFieldVariableAvailableProcessors[0];
			}).when(mockInstance).getAvailableProcessors();
			Mockito.doAnswer((stubInvo) -> {
				return mockFieldVariableDirectSize[0];
			}).when(mockInstance).getDirectMemorySizeBytes();
	...
	private static Injector makeInjector(int numProcessors, long directMemorySize, long heapSize, Properties props,
		...
		return Guice.createInjector(binder -> {
			binder.bind(RuntimeInfo.class)
					.toInstance(DruidProcessingConfigTest.mockRuntimeInfo1(numProcessors, directMemorySize, heapSize));
			binder.requestStaticInjection(JvmUtils.class);
			ConfigurationObjectFactory factory = Config.createFactory(props);
			DruidProcessingConfig config;
			if (replacements != null) {
				config = factory.buildWithReplacements(DruidProcessingConfig.class, replacements);
			} else {
	...
	public void testDefaultsLargeDirect() {
		...
		Assert.assertEquals(DruidProcessingConfig.MAX_DEFAULT_PROCESSING_BUFFER_SIZE_BYTES,
