...
public class ParallelCombinerTest {
	...
	private static final ReferenceCountingResourceHolder<ByteBuffer> TEST_RESOURCE_HOLDER = ConcurrentGrouperTest
			.mockReferenceCountingResourceHolder1(512);
	...
	public void testCombine() throws IOException {
		final ParallelCombiner<Long> combiner = new ParallelCombiner<>(TEST_RESOURCE_HOLDER,
				new AggregatorFactory[] { new CountAggregatorFactory("cnt").getCombiningFactory() }, KEY_SERDE_FACTORY,
				MoreExecutors.listeningDecorator(SERVICE), false, THREAD_NUM, 0, // default priority
				0, // default timeout
				4);
		...
		try (final CloseableIterator<Entry<Long>> iterator = combiner.combine(iterators, new ArrayList<>())) {
			...
			while (iterator.hasNext()) {
				Assert.assertEquals(new Entry<>(expectedKey, new Object[] { expectedKey++ * leafNum * 10 }),
						iterator.next());
			}
		}
