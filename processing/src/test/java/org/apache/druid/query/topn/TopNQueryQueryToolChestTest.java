...
public class TopNQueryQueryToolChestTest extends InitializedNullHandlingTest {
	...
	public void testMinTopNThreshold() {
		...
		try (CloseableStupidPool<ByteBuffer> pool = TestQueryRunners.createDefaultNonBlockingPool()) {
			...
			QueryRunner<Result<TopNResultValue>> mockRunner = Mockito.spy(QueryRunner.class);
			TopNQuery[] mockRunnerQuery = new TopNQuery[] { null };
			QueryRunner<Result<TopNResultValue>>[] mockRunnerRunner = new QueryRunner[1];
			mockRunnerRunner[0] = runner;
			try {
				Mockito.doAnswer((stubInvo) -> {
					QueryPlus<Result<TopNResultValue>> queryPlus = stubInvo.getArgument(0);
					ResponseContext responseContext = stubInvo.getArgument(1);
					mockRunnerQuery[0] = (TopNQuery) queryPlus.getQuery();
					return mockRunnerRunner[0].run(queryPlus, responseContext);
				}).when(mockRunner).run(Mockito.any(QueryPlus.class), Mockito.any(ResponseContext.class));
			} catch (Exception exception) {
			}
			new TopNQueryQueryToolChest.ThresholdAdjustingQueryRunner(mockRunner, config).run(QueryPlus.wrap(query1));
			Assert.assertEquals(1000, mockRunnerQuery[0].getThreshold());
			...
			Assert.assertEquals(500, mockRunnerQuery[0].getThreshold());
			...
			Assert.assertEquals(2000, mockRunnerQuery[0].getThreshold());
		}
	}
