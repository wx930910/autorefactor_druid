...
public abstract class MacroTestBase extends InitializedNullHandlingTest {
	...
	protected ExprEval<?> eval(final String expression, final Expr.ObjectBinding bindings) {
		...
		final ExprMacro wrappedMacro = Mockito.spy(ExprMacroTable.ExprMacro.class);
		AtomicLong wrappedMacroCalls = new AtomicLong();
		try {
			Mockito.doAnswer((stubInvo) -> {
				return macro.name();
			}).when(wrappedMacro).name();
			Mockito.doAnswer((stubInvo) -> {
				List<Expr> args = stubInvo.getArgument(0);
				wrappedMacroCalls.incrementAndGet();
				return macro.apply(args);
			}).when(wrappedMacro).apply(Mockito.any(List.class));
		...
		Assert.assertTrue("Calls made to macro.apply", wrappedMacroCalls.get() > 0);
