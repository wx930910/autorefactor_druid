...
public class LimitedSequenceTest {
	static public Accumulator<Integer, Integer> mockAccumulator1() {
		Accumulator<Integer, Integer> mockInstance = Mockito.spy(Accumulator.class);
		try {
			Mockito.doAnswer((stubInvo) -> {
				Integer accumulated = stubInvo.getArgument(0);
				Integer in = stubInvo.getArgument(1);
				return (Integer) accumulated + in;
			}).when(mockInstance).accumulate(Mockito.any(Integer.class), Mockito.any(Integer.class));
	...
	public void testSanityAccumulate() throws Exception {
		...
		SequenceTestHelper.testAll(Sequences.simple(NUMS).limit(threshold),
	...
	public void testWithYieldingSequence() {
		...
		// Create a Sequence whose Yielders will yield for each element, regardless of
		// what the accumulator passed
		// to "toYielder" does.
		final BaseSequence<Integer, Iterator<Integer>> sequence = new BaseSequence<Integer, Iterator<Integer>>(
			...
			public <OutType> Yielder<OutType> toYielder(final OutType initValue,
				...
				return super.toYielder(initValue, new DelegatingYieldingAccumulator<OutType, Integer>(accumulator) {
					@Override
		...
		// Can't use "testAll" because its "testYield" implementation depends on the
		// underlying Sequence _not_ yielding.
		SequenceTestHelper.testAccumulation("", sequence.limit(threshold),
				Lists.newArrayList(Iterables.limit(NUMS, threshold)));
	}
	...
	public void testNoSideEffects() {
		...
		final Sequence<Integer> seq = Sequences.simple(Iterables.transform(nums, input -> {
			accumulated.addAndGet(input);
			return input;
		})).limit(5);

		Assert.assertEquals(10, seq.accumulate(0, LimitedSequenceTest.mockAccumulator1()).intValue());
