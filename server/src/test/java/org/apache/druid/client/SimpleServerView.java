...
/**
 * A simple broker server view for testing which you can manually update the
 * server view.
 */
public class SimpleServerView implements TimelineServerView {
	...
	public void addServer(DruidServer server, DataSegment dataSegment) {
		servers.put(server,
				new QueryableDruidServer<>(server, new DirectDruidClient<>(warehouse, NOOP_QUERY_WATCHER, objectMapper,
						httpClient, server.getScheme(), server.getHost(), NoopServiceEmitter.mockServiceEmitter1())));
	...
	private void addSegmentToServer(DruidServer server, DataSegment segment) {
		final ServerSelector selector = selectors.computeIfAbsent(segment.getId().toString(),
	...
	public static DruidServer createServer(int nameSuiffix) {
		return new DruidServer("server_" + nameSuiffix, "127.0.0." + nameSuiffix, null, Long.MAX_VALUE,
				ServerType.HISTORICAL, "default", 0);
	}
}
