...
public class SQLMetadataRuleManagerTest {
	...
	public void setUp() {
		...
		auditManager = new SQLAuditManager(connector, Suppliers.ofInstance(tablesConfig),
				NoopServiceEmitter.mockServiceEmitter1(), mapper, new SQLAuditManagerConfig());

		connector.createRulesTable();
		ruleManager = new SQLMetadataRuleManager(mapper, new MetadataRuleManagerConfig(), tablesConfig, connector,
				auditManager);
	}
	...
	public void testRuleInsert() {
		List<Rule> rules = Collections.singletonList(new IntervalLoadRule(Intervals.of("2015-01-01/2015-02-01"),
		...
		// New rule should be be reflected in the in memory rules map immediately after
		// being set by user
