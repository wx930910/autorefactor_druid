...
public class SQLMetadataSupervisorManagerTest {
	static public SupervisorSpec mockSupervisorSpec1(String id, String dataSource) {
		String[] mockFieldVariableId = new String[1];
		String[] mockFieldVariableDataSource = new String[1];
		SupervisorSpec mockInstance = Mockito.spy(SupervisorSpec.class);
		mockFieldVariableId[0] = id;
		mockFieldVariableDataSource[0] = dataSource;
		try {
			Mockito.doAnswer((stubInvo) -> {
				return null;
			}).when(mockInstance).getSource();
			Mockito.doAnswer((stubInvo) -> {
				throw new UnsupportedOperationException();
			}).when(mockInstance).createSupervisor();
			Mockito.doAnswer((stubInvo) -> {
				return Collections.singletonList(mockFieldVariableDataSource[0]);
			}).when(mockInstance).getDataSources();
			Mockito.doAnswer((stubInvo) -> {
	...
	public void testInsertAndGet() {
		...
		Assert.assertEquals(data1rev1,
				((TestSupervisorSpec) supervisorSpecs.get(supervisor1).get(0).getSpec()).getData());
		Assert.assertEquals(data2rev1,
				((TestSupervisorSpec) supervisorSpecs.get(supervisor2).get(0).getSpec()).getData());
		...
		Assert.assertEquals(data1rev3,
				((TestSupervisorSpec) supervisorSpecs.get(supervisor1).get(0).getSpec()).getData());
		Assert.assertEquals(data1rev2,
				((TestSupervisorSpec) supervisorSpecs.get(supervisor1).get(1).getSpec()).getData());
		Assert.assertEquals(data1rev1,
				((TestSupervisorSpec) supervisorSpecs.get(supervisor1).get(2).getSpec()).getData());
		Assert.assertEquals(data2rev2,
				((TestSupervisorSpec) supervisorSpecs.get(supervisor2).get(0).getSpec()).getData());
		Assert.assertEquals(data2rev1,
				((TestSupervisorSpec) supervisorSpecs.get(supervisor2).get(1).getSpec()).getData());
	...
	public void testSkipDeserializingBadSpecs() {
		...
		supervisorManager.insert(supervisor1, new TestSupervisorSpec(supervisor1, data1rev1));
		supervisorManager.insert(supervisor2,
				SQLMetadataSupervisorManagerTest.mockSupervisorSpec1(supervisor2, supervisor2));
		supervisorManager.insert(supervisor1, new TestSupervisorSpec(supervisor1, data1rev2));
