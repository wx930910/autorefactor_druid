...
public class SqlSegmentsMetadataManagerTest {
	private static DataSegment createSegment(String dataSource, String interval, String version, String bucketKey,
			int binaryVersion) {
		return new DataSegment(dataSource, Intervals.of(interval), version,
				ImmutableMap.of("type", "s3_zip", "bucket", "test", "key", dataSource + "/" + bucketKey),
				ImmutableList.of("dim1", "dim2", "dim3"), ImmutableList.of("count", "value"), NoneShardSpec.instance(),
				binaryVersion, 1234L);
	}
	...
	private final DataSegment segment1 = createSegment("wikipedia", "2012-03-15T00:00:00.000/2012-03-16T00:00:00.000",
			"2012-03-16T00:36:30.848Z", "index/y=2012/m=03/d=15/2012-03-16T00:36:30.848Z/0/index.zip", 0);

	private final DataSegment segment2 = createSegment("wikipedia", "2012-01-05T00:00:00.000/2012-01-06T00:00:00.000",
			"2012-01-06T22:19:12.565Z", "wikipedia/index/y=2012/m=01/d=05/2012-01-06T22:19:12.565Z/0/index.zip", 0);
	...
	private void publish(DataSegment segment, boolean used) throws IOException {
		...
		publisher.publishSegment(segment.getId().toString(), segment.getDataSource(), DateTimes.nowUtc().toString(),
				segment.getInterval().getStart().toString(), segment.getInterval().getEnd().toString(), partitioned,
				segment.getVersion(), used, jsonMapper.writeValueAsBytes(segment));
	}
	...
	public void setUp() throws Exception {
		...
		sqlSegmentsMetadataManager = new SqlSegmentsMetadataManager(jsonMapper, Suppliers.ofInstance(config),
				derbyConnectorRule.metadataTablesConfigSupplier(), connector);
		sqlSegmentsMetadataManager.start();

		publisher = new SQLMetadataSegmentPublisher(jsonMapper, derbyConnectorRule.metadataTablesConfigSupplier().get(),
				connector);
	...
	public void testPollPeriodically() {
		...
		Assert.assertTrue(sqlSegmentsMetadataManager
				.getLatestDatabasePoll() instanceof SqlSegmentsMetadataManager.PeriodicDatabasePoll);
		...
		Assert.assertEquals(ImmutableList.of("wikipedia"), dataSourcesSnapshot.getDataSourcesWithAllUsedSegments()
				.stream().map(ImmutableDruidDataSource::getName).collect(Collectors.toList()));
	...
	public void testPollOnDemand() {
		DataSourcesSnapshot dataSourcesSnapshot = sqlSegmentsMetadataManager.getDataSourcesSnapshot();
		Assert.assertNull(dataSourcesSnapshot);
		// This should return false and not wait/poll anything as we did not schedule
		// periodic poll
		...
		Assert.assertTrue(sqlSegmentsMetadataManager
				.getLatestDatabasePoll() instanceof SqlSegmentsMetadataManager.OnDemandDatabasePoll);
		dataSourcesSnapshot = sqlSegmentsMetadataManager.getDataSourcesSnapshot();
		Assert.assertEquals(ImmutableSet.of("wikipedia"), sqlSegmentsMetadataManager.retrieveAllDataSourceNames());
		Assert.assertEquals(ImmutableList.of("wikipedia"), dataSourcesSnapshot.getDataSourcesWithAllUsedSegments()
				.stream().map(ImmutableDruidDataSource::getName).collect(Collectors.toList()));
	...
	public void testPollPeriodicallyAndOnDemandInterleave() throws Exception {
		...
		Assert.assertTrue(sqlSegmentsMetadataManager
				.getLatestDatabasePoll() instanceof SqlSegmentsMetadataManager.OnDemandDatabasePoll);
		// New datasource should now be in the snapshot since we just force on demand
		// poll.
		dataSourcesSnapshot = sqlSegmentsMetadataManager.getDataSourcesSnapshot();
		Assert.assertEquals(ImmutableList.of("wikipedia2", "wikipedia"),
				dataSourcesSnapshot.getDataSourcesWithAllUsedSegments().stream().map(ImmutableDruidDataSource::getName)
		...
		// This time wait for periodic poll (not doing on demand poll so we have to wait
		// a bit...)
		...
		Assert.assertTrue(sqlSegmentsMetadataManager
				.getLatestDatabasePoll() instanceof SqlSegmentsMetadataManager.PeriodicDatabasePoll);
		dataSourcesSnapshot = sqlSegmentsMetadataManager.getDataSourcesSnapshot();
		Assert.assertEquals(ImmutableList.of("wikipedia2", "wikipedia3", "wikipedia"),
				dataSourcesSnapshot.getDataSourcesWithAllUsedSegments().stream().map(ImmutableDruidDataSource::getName)
						.collect(Collectors.toList()));
	}
	...
	public void testPrepareImmutableDataSourceWithUsedSegmentsAwaitsPollOnRestart() throws IOException {
		...
		Assert.assertEquals(ImmutableSet.of(newSegment), ImmutableSet
				.copyOf(sqlSegmentsMetadataManager.getImmutableDataSourceWithUsedSegments("wikipedia2").getSegments()));
	}
	...
	public void testPrepareImmutableDataSourcesWithAllUsedSegmentsAwaitsPollOnRestart() throws IOException {
		DataSegment newSegment = pollThenStopThenStartIntro();
		Assert.assertEquals(ImmutableSet.of(segment1, segment2, newSegment),
				ImmutableSet.copyOf(sqlSegmentsMetadataManager.getImmutableDataSourcesWithAllUsedSegments().stream()
	...
	public void testPollWithCorruptedSegment() {
		...
		publisher.publishSegment("corrupt-segment-id", "corrupt-datasource", "corrupt-create-date",
				"corrupt-start-date", "corrupt-end-date", true, "corrupt-version", true,
		...
		Assert.assertEquals("wikipedia", Iterables
				.getOnlyElement(sqlSegmentsMetadataManager.getImmutableDataSourcesWithAllUsedSegments()).getName());
	}
	...
	public void testGetUnusedSegmentIntervals() {
		...
		Assert.assertEquals(ImmutableList.of(segment2.getInterval()),
				sqlSegmentsMetadataManager.getUnusedSegmentIntervals("wikipedia", DateTimes.of("3000"), 1));

		Assert.assertEquals(ImmutableList.of(segment2.getInterval(), segment1.getInterval()),
	...
	private static DataSegment createNewSegment1(String newDataSource) {
		return createSegment(newDataSource, "2017-10-15T00:00:00.000/2017-10-16T00:00:00.000",
				"2017-10-15T20:19:12.565Z", "index/y=2017/m=10/d=15/2017-10-16T20:19:12.565Z/0/index.zip", 0);
	}

	private static DataSegment createNewSegment2(String newDataSource) {
		return createSegment(newDataSource, "2017-10-17T00:00:00.000/2017-10-18T00:00:00.000",
				"2017-10-15T20:19:12.565Z", "index/y=2017/m=10/d=15/2017-10-16T20:19:12.565Z/0/index.zip", 0);
	}
	...
	public void testMarkSegmentAsUnused() throws IOException, InterruptedException {
		...
		final String newDataSource = "wikipedia2";
		final DataSegment newSegment = createSegment(newDataSource, "2017-10-15T00:00:00.000/2017-10-16T00:00:00.000",
				"2017-10-15T20:19:12.565Z", "wikipedia2/index/y=2017/m=10/d=15/2017-10-16T20:19:12.565Z/0/index.zip",
				0);
	...
	public void testMarkAsUsedNonOvershadowedSegments() throws Exception {
		...
		final String newDataSource = "wikipedia2";
		final DataSegment newSegment1 = createSegment(newDataSource, "2017-10-15T00:00:00.000/2017-10-17T00:00:00.000",
				"2017-10-15T20:19:12.565Z", "index/y=2017/m=10/d=15/2017-10-16T20:19:12.565Z/0/index.zip", 0);

		final DataSegment newSegment2 = createSegment(newDataSource, "2017-10-17T00:00:00.000/2017-10-18T00:00:00.000",
				"2017-10-16T20:19:12.565Z", "index/y=2017/m=10/d=15/2017-10-16T20:19:12.565Z/0/index.zip", 1);

		// Overshadowed by newSegment2
		final DataSegment newSegment3 = createSegment(newDataSource, "2017-10-17T00:00:00.000/2017-10-18T00:00:00.000",
				"2017-10-15T20:19:12.565Z", "index/y=2017/m=10/d=15/2017-10-16T20:19:12.565Z/0/index.zip", 1);

		publish(newSegment1, false);
		publish(newSegment2, false);
		publish(newSegment3, false);
		final ImmutableSet<String> segmentIds = ImmutableSet.of(newSegment1.getId().toString(),
				newSegment2.getId().toString(), newSegment3.getId().toString());
		...
		sqlSegmentsMetadataManager.poll();
		Assert.assertEquals(ImmutableSet.of(segment1, segment2, newSegment1, newSegment2),
				ImmutableSet.copyOf(sqlSegmentsMetadataManager.iterateAllUsedSegments()));
	}
	...
	public void testMarkAsUsedNonOvershadowedSegmentsInInterval() throws IOException {
		...
		final DataSegment newSegment3 = createSegment(newDataSource, "2017-10-19T00:00:00.000/2017-10-20T00:00:00.000",
				"2017-10-15T20:19:12.565Z", "index/y=2017/m=10/d=15/2017-10-16T20:19:12.565Z/0/index.zip", 0);
	...
	public void testMarkAsUsedNonOvershadowedSegmentsInIntervalWithOverlappingInterval() throws IOException {
		...
		final DataSegment newSegment3 = createSegment(newDataSource, "2017-10-19T00:00:00.000/2017-10-22T00:00:00.000",
				"2017-10-15T20:19:12.565Z", "index/y=2017/m=10/d=15/2017-10-16T20:19:12.565Z/0/index.zip", 0);
		...
		// 1 out of 3 segments match the interval, other 2 overlap, only the segment
		// fully contained will be marked unused
		Assert.assertEquals(1,
				sqlSegmentsMetadataManager.markAsUsedNonOvershadowedSegmentsInInterval(newDataSource, theInterval));

		sqlSegmentsMetadataManager.poll();
		Assert.assertEquals(ImmutableSet.of(segment1, segment2, newSegment2),
				ImmutableSet.copyOf(sqlSegmentsMetadataManager.iterateAllUsedSegments()));
	}
	...
	public void testMarkSegmentsAsUnused() throws IOException {
		...
		Assert.assertEquals(segmentIds.size(),
				sqlSegmentsMetadataManager.markSegmentsAsUnused(newDataSource, segmentIds));
		sqlSegmentsMetadataManager.poll();
		Assert.assertEquals(ImmutableSet.of(segment1, segment2),
				ImmutableSet.copyOf(sqlSegmentsMetadataManager.iterateAllUsedSegments()));
	}
	...
	public void testMarkAsUnusedSegmentsInInterval() throws IOException {
		...
		sqlSegmentsMetadataManager.poll();
		Assert.assertEquals(ImmutableSet.of(segment1, segment2, newSegment3),
				ImmutableSet.copyOf(sqlSegmentsMetadataManager.iterateAllUsedSegments()));
	}
	...
	public void testMarkAsUnusedSegmentsInIntervalWithOverlappingInterval() throws IOException {
		...
		sqlSegmentsMetadataManager.poll();
		Assert.assertEquals(ImmutableSet.of(segment1, segment2, newSegment1, newSegment3),
				ImmutableSet.copyOf(sqlSegmentsMetadataManager.iterateAllUsedSegments()));
	}
	...
	public void testIterateAllUsedNonOvershadowedSegmentsForDatasourceInterval() throws Exception {
		...
		Optional<Iterable<DataSegment>> segments = sqlSegmentsMetadataManager
				.iterateAllUsedNonOvershadowedSegmentsForDatasourceInterval("wikipedia", theInterval, true);
		...
		final DataSegment newSegment2 = createSegment("wikipedia", "2012-03-16T00:00:00.000/2012-03-17T00:00:00.000",
				"2017-10-15T20:19:12.565Z", "index/y=2017/m=10/d=15/2017-10-16T20:19:12.565Z/0/index.zip", 0);
		publisher.publishSegment(newSegment2);

		// New segment is not returned since we call without force poll
		segments = sqlSegmentsMetadataManager.iterateAllUsedNonOvershadowedSegmentsForDatasourceInterval("wikipedia",
				theInterval, false);
		Assert.assertTrue(segments.isPresent());
