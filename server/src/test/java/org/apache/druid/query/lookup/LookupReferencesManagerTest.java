...
public class LookupReferencesManagerTest {
	...
	public void setUp() throws IOException {
		...
		lookupReferencesManager = new LookupReferencesManager(
				new LookupConfig(temporaryFolder.newFolder().getAbsolutePath()), mapper, druidLeaderClient, config,
				true);
	}
	...
	public void testStartStop() throws InterruptedException, IOException {
		lookupReferencesManager = new LookupReferencesManager(new LookupConfig(null), mapper, druidLeaderClient,
				config);
		...
		EasyMock.expect(druidLeaderClient.makeRequest(HttpMethod.GET,
				"/druid/coordinator/v1/lookups/config/lookupTier?detailed=true")).andReturn(request);
		StringFullResponseHolder responseHolder = new StringFullResponseHolder(HttpResponseStatus.OK,
				newEmptyResponse(), StandardCharsets.UTF_8).addChunk(strResult);
	...
	public void testAddGetRemove() throws Exception {
		...
		LookupExtractorFactoryContainer testContainer = new LookupExtractorFactoryContainer("0",
	...
	public void testGetAllLookupNames() throws Exception {
		LookupExtractorFactoryContainer container1 = new LookupExtractorFactoryContainer("0",
				new MapLookupExtractorFactory(ImmutableMap.of("key1", "value1"), true));

		LookupExtractorFactoryContainer container2 = new LookupExtractorFactoryContainer("0",
	...
	public void testGetAllLookupsState() throws Exception {
		...
		LookupExtractorFactoryContainer container3 = new LookupExtractorFactoryContainer("0",
	...
	public void testRealModeWithMainThread() throws Exception {
		...
		Assert.assertEquals(ImmutableSet.of("test", "testMockForRealModeWithMainThread"),
		...
		Assert.assertEquals(ImmutableSet.of("testMockForRealModeWithMainThread"),
				lookupReferencesManager.getAllLookupNames());
	...
	public void testLoadLookupOnCoordinatorFailure() throws Exception {
		...
		lookupReferencesManager = new LookupReferencesManager(lookupConfig, mapper, druidLeaderClient, config);

		Request request = new Request(HttpMethod.GET, new URL("http://localhost:1234/xx"));
		EasyMock.expect(config.getLookupTier()).andReturn(LOOKUP_TIER).anyTimes();
		EasyMock.replay(config);
		EasyMock.expect(druidLeaderClient.makeRequest(HttpMethod.GET,
				"/druid/coordinator/v1/lookups/config/lookupTier?detailed=true")).andReturn(request).anyTimes();
		...
		lookupConfig = new LookupConfig(
				lookupReferencesManager.lookupSnapshotTaker.getPersistFile(LOOKUP_TIER).getParent()) {
			@Override
			public int getCoordinatorRetryDelay() {
				return 10;
			}
		...
		lookupReferencesManager = new LookupReferencesManager(lookupConfig, mapper, druidLeaderClient, config, true);
		...
		EasyMock.replay(druidLeaderClient);
		lookupReferencesManager.start();
		Assert.assertEquals(Optional.of(container),
	...
	public void testDisableLookupSync() throws Exception {
		...
		LookupReferencesManager lookupReferencesManager = new LookupReferencesManager(lookupConfig, mapper,
				druidLeaderClient, config);
		Map<String, Object> lookupMap = new HashMap<>();
