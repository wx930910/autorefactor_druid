...
public class SegmentManagerThreadSafetyTest {
	...
	public void setup() throws IOException {
		...
		objectMapper = new DefaultObjectMapper()
				.registerModule(new SimpleModule().registerSubtypes(new NamedType(LocalLoadSpec.class, "local"),
						new NamedType(TestSegmentizerFactory.class, "test")))
		...
		segmentLoader = new SegmentLoaderLocalCacheManager(indexIO, new SegmentLoaderConfig() {
			@Override
	...
	public void testLoadSameSegment() throws IOException, ExecutionException, InterruptedException {
		...
		final List<Future> futures = IntStream.range(0, 16)
	...
	private DataSegment createSegment(String interval) throws IOException {
		final DataSegment tmpSegment = new DataSegment("dataSource", Intervals.of(interval), "version",
				Collections.emptyMap(), Collections.emptyList(), Collections.emptyList(), new NumberedShardSpec(0, 0),
				9, 100);
