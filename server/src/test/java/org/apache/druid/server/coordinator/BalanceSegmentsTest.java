...
public class BalanceSegmentsTest {
	static public BalancerStrategy mockBalancerStrategy1(BalancerStrategy delegate,
			List<BalancerSegmentHolder> pickOrder) {
		BalancerStrategy[] mockFieldVariableDelegate = new BalancerStrategy[1];
		AtomicInteger mockFieldVariablePickCounter = new AtomicInteger(0);
		List<BalancerSegmentHolder>[] mockFieldVariablePickOrder = new List[1];
		BalancerStrategy mockInstance = Mockito.spy(BalancerStrategy.class);
		mockFieldVariableDelegate[0] = delegate;
		mockFieldVariablePickOrder[0] = pickOrder;
		try {
			Mockito.doAnswer((stubInvo) -> {
				String tier = stubInvo.getArgument(0);
				CoordinatorStats stats = stubInvo.getArgument(1);
				List<ServerHolder> serverHolderList = stubInvo.getArgument(2);
				mockFieldVariableDelegate[0].emitStats(tier, stats, serverHolderList);
				return null;
			}).when(mockInstance).emitStats(Mockito.any(String.class), Mockito.any(CoordinatorStats.class),
					Mockito.any(List.class));
			Mockito.doAnswer((stubInvo) -> {
				DataSegment proposalSegment = stubInvo.getArgument(0);
				List<ServerHolder> serverHolders = stubInvo.getArgument(1);
				return mockFieldVariableDelegate[0].findNewSegmentHomeBalancer(proposalSegment, serverHolders);
			}).when(mockInstance).findNewSegmentHomeBalancer(Mockito.any(DataSegment.class), Mockito.any(List.class));
			Mockito.doAnswer((stubInvo) -> {
				DataSegment proposalSegment = stubInvo.getArgument(0);
				List<ServerHolder> serverHolders = stubInvo.getArgument(1);
				return mockFieldVariableDelegate[0].findNewSegmentHomeReplicator(proposalSegment, serverHolders);
			}).when(mockInstance).findNewSegmentHomeReplicator(Mockito.any(DataSegment.class), Mockito.any(List.class));
			Mockito.doAnswer((stubInvo) -> {
				return mockFieldVariablePickOrder[0]
						.get(mockFieldVariablePickCounter.getAndIncrement() % mockFieldVariablePickOrder[0].size());
			}).when(mockInstance).pickSegmentToMove(Mockito.any(List.class), Mockito.any(Set.class));
	...
	public void setUp() {
		...
		segment1 = new DataSegment("datasource1", new Interval(start1, start1.plusHours(1)), version.toString(),
				new HashMap<>(), new ArrayList<>(), new ArrayList<>(), NoneShardSpec.instance(), 0, 11L);
		segment2 = new DataSegment("datasource1", new Interval(start2, start2.plusHours(1)), version.toString(),
				new HashMap<>(), new ArrayList<>(), new ArrayList<>(), NoneShardSpec.instance(), 0, 7L);
		segment3 = new DataSegment("datasource2", new Interval(start1, start1.plusHours(1)), version.toString(),
				new HashMap<>(), new ArrayList<>(), new ArrayList<>(), NoneShardSpec.instance(), 0, 4L);
		segment4 = new DataSegment("datasource2", new Interval(start2, start2.plusHours(1)), version.toString(),
				new HashMap<>(), new ArrayList<>(), new ArrayList<>(), NoneShardSpec.instance(), 0, 8L);
		segment5 = new DataSegment("datasourceBroadcast", new Interval(start2, start2.plusHours(1)), version.toString(),
				new HashMap<>(), new ArrayList<>(), new ArrayList<>(), NoneShardSpec.instance(), 0, 8L);
	...
	public void testMoveToEmptyServerBalancer() {
		...
		BalancerStrategy predefinedPickOrderStrategy = BalanceSegmentsTest.mockBalancerStrategy1(balancerStrategy,
				ImmutableList.of(new BalancerSegmentHolder(druidServer1, segment1),
		...
		DruidCoordinatorRuntimeParams params = defaultRuntimeParamsBuilder(ImmutableList.of(druidServer1, druidServer2),
				ImmutableList.of(peon1, peon2)).withBalancerStrategy(predefinedPickOrderStrategy)
						.withBroadcastDatasources(broadcastDatasources).build();
	...
	/**
	 * Server 1 has 2 segments. Server 2 (decommissioning) has 2 segments. Server 3
	 * is empty. Decommissioning percent is 60. Max segments to move is 3. 2 (of 2)
	 * segments should be moved from Server 2 and 1 (of 2) from Server 1.
	 */
	@Test
	public void testMoveDecommissioningMaxPercentOfMaxSegmentsToMove() {
		...
		EasyMock.expect(strategy.pickSegmentToMove(ImmutableList.of(new ServerHolder(druidServer2, peon2, false)),
				broadcastDatasources)).andReturn(new BalancerSegmentHolder(druidServer2, segment3))
		...
		EasyMock.expect(strategy.findNewSegmentHomeBalancer(EasyMock.anyObject(), EasyMock.anyObject()))
				.andReturn(new ServerHolder(druidServer3, peon3)).anyTimes();
		EasyMock.replay(strategy);

		DruidCoordinatorRuntimeParams params = defaultRuntimeParamsBuilder(
				ImmutableList.of(druidServer1, druidServer2, druidServer3), ImmutableList.of(peon1, peon2, peon3),
				ImmutableList.of(false, true, false))
						.withDynamicConfigs(CoordinatorDynamicConfig.builder().withMaxSegmentsToMove(3)
								.withDecommissioningMaxPercentOfMaxSegmentsToMove(60).build() // ceil(3 * 0.6) = 2
																								// segments from
																								// decommissioning
																								// servers
						).withBalancerStrategy(strategy).withBroadcastDatasources(broadcastDatasources).build();
	...
	public void testMoveDecommissioningMaxPercentOfMaxSegmentsToMoveWithNoDecommissioning() {
		...
		DruidCoordinatorRuntimeParams params = defaultRuntimeParamsBuilder(
				...
				ImmutableList.of(false, false, false))
						.withDynamicConfigs(CoordinatorDynamicConfig.builder().withMaxSegmentsToMove(3)
								.withDecommissioningMaxPercentOfMaxSegmentsToMove(9).build())
						.withBalancerStrategy(strategy).withBroadcastDatasources(broadcastDatasources).build();
	...
	public void testMoveToDecommissioningServer() {
		...
		BalancerStrategy strategy = EasyMock.createMock(BalancerStrategy.class);
		EasyMock.expect(strategy.pickSegmentToMove(EasyMock.anyObject(), EasyMock.anyObject()))
				.andReturn(new BalancerSegmentHolder(druidServer1, segment1)).anyTimes();
		EasyMock.expect(strategy.findNewSegmentHomeBalancer(EasyMock.anyObject(), EasyMock.anyObject()))
				.andAnswer(() -> {
		...
		DruidCoordinatorRuntimeParams params = defaultRuntimeParamsBuilder(ImmutableList.of(druidServer1, druidServer2),
				ImmutableList.of(peon1, peon2), ImmutableList.of(false, true)).withBalancerStrategy(strategy)
						.withBroadcastDatasources(broadcastDatasources).build();
	...
	public void testMoveFromDecommissioningServer() {
		...
		BalancerStrategy strategy = EasyMock.createMock(BalancerStrategy.class);
		EasyMock.expect(strategy.pickSegmentToMove(EasyMock.anyObject(), EasyMock.anyObject()))
				.andReturn(new BalancerSegmentHolder(druidServer1, segment1)).once();
		EasyMock.expect(strategy.findNewSegmentHomeBalancer(EasyMock.anyObject(), EasyMock.anyObject()))
				.andReturn(holder2).once();
		EasyMock.replay(strategy);

		DruidCoordinatorRuntimeParams params = defaultRuntimeParamsBuilder(ImmutableList.of(druidServer1, druidServer2),
				ImmutableList.of(peon1, peon2), ImmutableList.of(true, false))
	...
	public void testMoveMaxLoadQueueServerBalancer() {
		...
		DruidCoordinatorRuntimeParams params = defaultRuntimeParamsBuilder(ImmutableList.of(druidServer1, druidServer2),
				ImmutableList.of(peon1, peon2)).withBalancerStrategy(predefinedPickOrderStrategy)
						...
						.withDynamicConfigs(
								CoordinatorDynamicConfig.builder().withMaxSegmentsToMove(MAX_SEGMENTS_TO_MOVE)
										.withMaxSegmentsInNodeLoadingQueue(1).build())
						.build();

		params = new BalanceSegmentsTester(coordinator).run(params);

		// max to move is 5, all segments on server 1, but only expect to move 1 to
		// server 2 since max node load queue is 1
		Assert.assertEquals(1, params.getCoordinatorStats().getTieredStat("movedCount", "normal"));
	}
	...
	public void testMoveSameSegmentTwice() {
		...
		DruidCoordinatorRuntimeParams params = defaultRuntimeParamsBuilder(ImmutableList.of(druidServer1, druidServer2),
				ImmutableList.of(peon1, peon2)).withBalancerStrategy(predefinedPickOrderStrategy)
						.withBroadcastDatasources(broadcastDatasources)
						.withDynamicConfigs(CoordinatorDynamicConfig.builder().withMaxSegmentsToMove(2).build())
						.build();
	...
	public void testRun1() {
		...
		DruidCoordinatorRuntimeParams params = defaultRuntimeParamsBuilder(ImmutableList.of(druidServer1, druidServer2),
				ImmutableList.of(peon1, peon2)).build();
	...
	private DruidCoordinatorRuntimeParams.Builder defaultRuntimeParamsBuilder(List<ImmutableDruidServer> druidServers,
			List<LoadQueuePeon> peons) {
		return defaultRuntimeParamsBuilder(druidServers, peons,
				druidServers.stream().map(s -> false).collect(Collectors.toList()));
	}

	private DruidCoordinatorRuntimeParams.Builder defaultRuntimeParamsBuilder(List<ImmutableDruidServer> druidServers,
			List<LoadQueuePeon> peons, List<Boolean> decommissioning) {
		return CoordinatorRuntimeParamsTestHelpers.newBuilder().withDruidCluster(DruidClusterBuilder.newBuilder()
				.addTier("normal", IntStream.range(0, druidServers.size())
						.mapToObj(i -> new ServerHolder(druidServers.get(i), peons.get(i), decommissioning.get(i)))
						.toArray(ServerHolder[]::new))
				.build())
				.withLoadManagementPeons(IntStream.range(0, peons.size()).boxed()
				...
				.withBroadcastDatasources(broadcastDatasources).withBalancerStrategy(balancerStrategy);
	}

	private static void mockDruidServer(ImmutableDruidServer druidServer, String name, String tier, long currentSize,
			long maxSize, List<DataSegment> segments) {
	...
	private static void mockCoordinator(DruidCoordinator coordinator) {
		coordinator.moveSegment(EasyMock.anyObject(), EasyMock.anyObject(), EasyMock.anyObject(), EasyMock.anyObject(),
				EasyMock.anyObject());
	...
	private DruidCoordinatorRuntimeParams setupParamsForDecommissioningMaxPercentOfMaxSegmentsToMove(int percent) {
		...
		// either decommissioning servers list or acitve ones (ie servers list is [2] or
		// [1, 3])
		BalancerStrategy strategy = EasyMock.createMock(BalancerStrategy.class);
		EasyMock.expect(strategy.pickSegmentToMove(ImmutableList.of(new ServerHolder(druidServer2, peon2, true)),
				broadcastDatasources)).andReturn(new BalancerSegmentHolder(druidServer2, segment2));
		EasyMock.expect(strategy.pickSegmentToMove(EasyMock.anyObject(), EasyMock.anyObject()))
				.andReturn(new BalancerSegmentHolder(druidServer1, segment1));
		EasyMock.expect(strategy.findNewSegmentHomeBalancer(EasyMock.anyObject(), EasyMock.anyObject()))
				.andReturn(new ServerHolder(druidServer3, peon3)).anyTimes();
		...
		return defaultRuntimeParamsBuilder(ImmutableList.of(druidServer1, druidServer2, druidServer3),
				ImmutableList.of(peon1, peon2, peon3), ImmutableList.of(false, true, false))
						.withDynamicConfigs(CoordinatorDynamicConfig.builder().withMaxSegmentsToMove(1)
								.withDecommissioningMaxPercentOfMaxSegmentsToMove(percent).build())
						.withBalancerStrategy(strategy).withBroadcastDatasources(broadcastDatasources).build();
	}
}
