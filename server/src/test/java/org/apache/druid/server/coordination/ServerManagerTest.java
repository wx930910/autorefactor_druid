...
public class ServerManagerTest {
	...
	public void setUp() {
		...
		segmentManager = new SegmentManager(new SegmentLoader() {
			...
			public Segment getSegment(final DataSegment segment, boolean lazy) {
				return new SegmentForTesting(MapUtils.getString(segment.getLoadSpec(), "version"),
		...
		}, NoopServiceEmitter.mockServiceEmitter1(), serverManagerExec,
				new ForegroundCachePopulator(new DefaultObjectMapper(), new CachePopulatorStats(), -1),
				new DefaultObjectMapper(), new LocalCacheProvider().get(), new CacheConfig(), segmentManager,
	...
	public void testSimpleGet() {
		Future future = assertQueryable(Granularities.DAY, "test", Intervals.of("P1d/2011-04-01"),
				ImmutableList.of(new Pair<String, Interval>("1", Intervals.of("P1d/2011-04-01"))));
		waitForTestVerificationAndCleanup(future);

		future = assertQueryable(Granularities.DAY, "test", Intervals.of("P2d/2011-04-02"),
				ImmutableList.of(new Pair<String, Interval>("1", Intervals.of("P1d/2011-04-01")),
	...
	public void testDelete1() {
		...
		Future future = assertQueryable(Granularities.DAY, dataSouce, interval,
				ImmutableList.of(new Pair<String, Interval>("2", interval)));
		waitForTestVerificationAndCleanup(future);

		dropQueryable(dataSouce, "2", interval);
		future = assertQueryable(Granularities.DAY, dataSouce, interval,
				ImmutableList.of(new Pair<String, Interval>("1", interval)));
		waitForTestVerificationAndCleanup(future);
	}
	...
	public void testDelete2() {
		...
		Future future = assertQueryable(Granularities.DAY, "test", Intervals.of("2011-04-04/2011-04-06"),
				ImmutableList.of(new Pair<String, Interval>("3", Intervals.of("2011-04-04/2011-04-05"))));
		waitForTestVerificationAndCleanup(future);

		dropQueryable("test", "3", Intervals.of("2011-04-04/2011-04-05"));
		dropQueryable("test", "1", Intervals.of("2011-04-04/2011-04-05"));

		future = assertQueryable(Granularities.HOUR, "test", Intervals.of("2011-04-04/2011-04-04T06"),
				ImmutableList.of(new Pair<String, Interval>("2", Intervals.of("2011-04-04T00/2011-04-04T01")),
		...
		future = assertQueryable(Granularities.HOUR, "test", Intervals.of("2011-04-04/2011-04-04T03"),
				ImmutableList.of(new Pair<String, Interval>("2", Intervals.of("2011-04-04T00/2011-04-04T01")),
						new Pair<String, Interval>("2", Intervals.of("2011-04-04T01/2011-04-04T02")),
						new Pair<String, Interval>("2", Intervals.of("2011-04-04T02/2011-04-04T03"))));
		waitForTestVerificationAndCleanup(future);

		future = assertQueryable(Granularities.HOUR, "test", Intervals.of("2011-04-04T04/2011-04-04T06"),
				ImmutableList.of(new Pair<String, Interval>("2", Intervals.of("2011-04-04T04/2011-04-04T05")),
						new Pair<String, Interval>("2", Intervals.of("2011-04-04T05/2011-04-04T06"))));
		waitForTestVerificationAndCleanup(future);
	}
	...
	private Future assertQueryable(Granularity granularity, String dataSource, Interval interval,
		...
		final SearchQuery query = Druids.newSearchQueryBuilder().dataSource(dataSource).intervals(intervals)
				.granularity(granularity).limit(10000).query("wow").build();
		final QueryRunner<Result<SearchResultValue>> runner = serverManager.getQueryRunnerForIntervals(query,
				intervals);
	...
	public void loadQueryable(String dataSource, String version, Interval interval) {
		try {
			segmentManager.loadSegment(new DataSegment(dataSource, interval, version,
					ImmutableMap.of("version", version, "interval", interval), Arrays.asList("dim1", "dim2", "dim3"),
					Arrays.asList("metric1", "metric2"), NoneShardSpec.instance(), IndexIO.CURRENT_VERSION_ID, 123L),
					false);
	...
	public void dropQueryable(String dataSource, String version, Interval interval) {
		segmentManager.dropSegment(new DataSegment(dataSource, interval, version,
				ImmutableMap.of("version", version, "interval", interval), Arrays.asList("dim1", "dim2", "dim3"),
				Arrays.asList("metric1", "metric2"), NoneShardSpec.instance(), IndexIO.CURRENT_VERSION_ID, 123L));
	}

	public static class MyQueryRunnerFactory implements QueryRunnerFactory<Result<SearchResultValue>, SearchQuery> {
		static public QueryRunner<Result<SearchResultValue>> mockQueryRunner1(
				QueryRunner<Result<SearchResultValue>> runner, CountDownLatch waitLatch, CountDownLatch waitYieldLatch,
				CountDownLatch notifyLatch) {
			QueryRunner<Result<SearchResultValue>>[] mockFieldVariableRunner = new QueryRunner[1];
			CountDownLatch[] mockFieldVariableWaitLatch = new CountDownLatch[1];
			CountDownLatch[] mockFieldVariableNotifyLatch = new CountDownLatch[1];
			CountDownLatch[] mockFieldVariableWaitYieldLatch = new CountDownLatch[1];
			QueryRunner<Result<SearchResultValue>> mockInstance = Mockito.spy(QueryRunner.class);
			mockFieldVariableRunner[0] = runner;
			mockFieldVariableWaitLatch[0] = waitLatch;
			mockFieldVariableWaitYieldLatch[0] = waitYieldLatch;
			mockFieldVariableNotifyLatch[0] = notifyLatch;
			try {
				Mockito.doAnswer((stubInvo) -> {
					QueryPlus<Result<SearchResultValue>> queryPlus = stubInvo.getArgument(0);
					ResponseContext responseContext = stubInvo.getArgument(1);
					return new BlockingSequence<>(mockFieldVariableRunner[0].run(queryPlus, responseContext),
							mockFieldVariableWaitLatch[0], mockFieldVariableWaitYieldLatch[0],
							mockFieldVariableNotifyLatch[0]);
				}).when(mockInstance).run(Mockito.any(QueryPlus.class), Mockito.any(ResponseContext.class));
			} catch (Exception exception) {
			}
			return mockInstance;
		}

		static public QueryToolChest<Result<SearchResultValue>, SearchQuery> mockQueryToolChest1() {
			QueryToolChest<Result<SearchResultValue>, SearchQuery> mockInstance = Mockito.spy(QueryToolChest.class);
			try {
				Mockito.doAnswer((stubInvo) -> {
					return Functions.identity();
				}).when(mockInstance).makePreComputeManipulatorFn(Mockito.any(SearchQuery.class),
						Mockito.any(MetricManipulationFn.class));
				Mockito.doAnswer((stubInvo) -> {
					return new DefaultQueryMetrics<>();
				}).when(mockInstance).makeMetrics(Mockito.any(SearchQuery.class));
				Mockito.doAnswer((stubInvo) -> {
					QueryRunner<Result<SearchResultValue>> runner = stubInvo.getArgument(0);
					return runner;
				}).when(mockInstance).mergeResults(Mockito.any(QueryRunner.class));
				Mockito.doAnswer((stubInvo) -> {
					return new TypeReference<Result<SearchResultValue>>() {
					};
				}).when(mockInstance).getResultTypeReference();
			} catch (Exception exception) {
			}
			return mockInstance;
		}
		...
		public MyQueryRunnerFactory(CountDownLatch waitLatch, CountDownLatch waitYieldLatch,
				CountDownLatch notifyLatch) {
		...
		public QueryRunner<Result<SearchResultValue>> createRunner(Segment adapter) {
			...
			return MyQueryRunnerFactory.mockQueryRunner1(new NoopQueryRunner<>(), waitLatch, waitYieldLatch,
					notifyLatch);
		}

		@Override
		public QueryRunner<Result<SearchResultValue>> mergeRunners(ExecutorService queryExecutor,
		...
		public QueryToolChest<Result<SearchResultValue>, SearchQuery> getToolchest() {
			return MyQueryRunnerFactory.mockQueryToolChest1();
		}
	...
	private static class BlockingSequence<T> extends YieldingSequenceBase<T> {
		...
		private BlockingSequence(Sequence<T> baseSequence, CountDownLatch waitLatch, CountDownLatch waitYieldLatch,
				CountDownLatch notifyLatch) {
