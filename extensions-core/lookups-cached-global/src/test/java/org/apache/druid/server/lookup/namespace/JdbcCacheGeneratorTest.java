...
public class JdbcCacheGeneratorTest {
	private static final MetadataStorageConnectorConfig MISSING_METADATA_STORAGE_CONNECTOR_CONFIG = createMetadataStorageConnectorConfig(
			"postgresql");

	private static final CacheScheduler.EntryImpl<JdbcExtractionNamespace> KEY = EasyMock
			.mock(CacheScheduler.EntryImpl.class);

	private static final ServiceEmitter SERVICE_EMITTER = NoopServiceEmitter.mockServiceEmitter1();

	private static final CacheScheduler SCHEDULER = new CacheScheduler(SERVICE_EMITTER, Collections.emptyMap(),
			new OnHeapNamespaceExtractionCacheManager(new Lifecycle(), SERVICE_EMITTER,
					new NamespaceExtractionConfig()));

	private static final String LAST_VERSION = "1";

	private static final String MISSING_JDB_DRIVER_JAR_MSG = "JDBC driver JAR files missing from extensions/druid-lookups-cached-global directory";
	...
	public void indicatesMissingJdbcJarsWithTsColumn() {
		...
		JdbcExtractionNamespace missingJarNamespace = createJdbcExtractionNamespace(
				MISSING_METADATA_STORAGE_CONNECTOR_CONFIG, tsColumn);
	...
	public void indicatesMissingJdbcJarsWithoutTsColumn() {
		...
		JdbcExtractionNamespace missingJarNamespace = createJdbcExtractionNamespace(
				MISSING_METADATA_STORAGE_CONNECTOR_CONFIG, missingTsColumn);
	...
	private static JdbcExtractionNamespace createJdbcExtractionNamespace(
			MetadataStorageConnectorConfig metadataStorageConnectorConfig, @Nullable String tsColumn) {
		return new JdbcExtractionNamespace(metadataStorageConnectorConfig, "table", "keyColumn", "valueColumn",
				tsColumn, "filter", Period.ZERO);
	}
}
