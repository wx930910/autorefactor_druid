...
public class CoordinatorBasicAuthorizerMetadataStorageUpdaterTest {
	...
	private static final Map<String, BasicAuthorizerUser> BASE_USER_MAP = ImmutableMap.of(BasicAuthUtils.ADMIN_NAME,
			new BasicAuthorizerUser(BasicAuthUtils.ADMIN_NAME, ImmutableSet.of(BasicAuthUtils.ADMIN_NAME)),
			BasicAuthUtils.INTERNAL_USER_NAME, new BasicAuthorizerUser(BasicAuthUtils.INTERNAL_USER_NAME,
					ImmutableSet.of(BasicAuthUtils.INTERNAL_USER_NAME)));

	private static final Map<String, BasicAuthorizerRole> BASE_ROLE_MAP = ImmutableMap.of(BasicAuthUtils.ADMIN_NAME,
			new BasicAuthorizerRole(BasicAuthUtils.ADMIN_NAME,
			...
			new BasicAuthorizerRole(BasicAuthUtils.INTERNAL_USER_NAME, BasicAuthorizerPermission
					.makePermissionList(CoordinatorBasicAuthorizerMetadataStorageUpdater.SUPERUSER_PERMISSIONS)));
	...
	public void setUp() {
		...
		updater = new CoordinatorBasicAuthorizerMetadataStorageUpdater(
				new AuthorizerMapper(ImmutableMap.of(AUTHORIZER_NAME,
						new BasicRoleBasedAuthorizer(null, AUTHORIZER_NAME, null, null, null, null, null, null))),
				connector, tablesConfig, new BasicAuthCommonCacheConfig(null, null, null, null), objectMapper,
	...
	public void testCreateDeleteUser() {
		...
		Map<String, BasicAuthorizerUser> actualUserMap = BasicAuthUtils.deserializeAuthorizerUserMap(objectMapper,
		...
		actualUserMap = BasicAuthUtils.deserializeAuthorizerUserMap(objectMapper,
				updater.getCurrentUserMapBytes(AUTHORIZER_NAME));
		Assert.assertEquals(expectedUserMap, actualUserMap);
	}
	...
	public void testCreateDeleteGroupMapping() {
		...
		Map<String, BasicAuthorizerGroupMapping> actualGroupMappingMap = BasicAuthUtils
				.deserializeAuthorizerGroupMappingMap(objectMapper,
		...
		actualGroupMappingMap = BasicAuthUtils.deserializeAuthorizerGroupMappingMap(objectMapper,
				updater.getCurrentGroupMappingMapBytes(AUTHORIZER_NAME));
		Assert.assertEquals(expectedGroupMappingMap, actualGroupMappingMap);
	}
	...
	public void testCreateDeleteRole() {
		...
		Map<String, BasicAuthorizerRole> actualRoleMap = BasicAuthUtils.deserializeAuthorizerRoleMap(objectMapper,
		...
		actualRoleMap = BasicAuthUtils.deserializeAuthorizerRoleMap(objectMapper,
				updater.getCurrentRoleMapBytes(AUTHORIZER_NAME));
		Assert.assertEquals(expectedRoleMap, actualRoleMap);
	}
	...
	public void testAddAndRemoveRoleToGroupMapping() {
		...
		Map<String, BasicAuthorizerGroupMapping> expectedGroupMappingMap = new HashMap<>();
		expectedGroupMappingMap.put("druid",
				new BasicAuthorizerGroupMapping("druid", "CN=test", ImmutableSet.of("druidRole")));
	...
	public void testAddExistingRoleToGroupMappingWithRoleFails() {
		expectedException.expect(BasicSecurityDBResourceException.class);
		expectedException.expectMessage("Group mapping [druid] already has role [druidRole].");
		updater.createGroupMapping(AUTHORIZER_NAME,
				new BasicAuthorizerGroupMapping("druid", "CN=test", ImmutableSet.of("druidRole")));
		updater.createRole(AUTHORIZER_NAME, "druidRole");
		updater.assignGroupMappingRole(AUTHORIZER_NAME, "druid", "druidRole");
	}
	...
	public void testSetRolePermissions() {
		...
		List<ResourceAction> permsToAdd = ImmutableList
				.of(new ResourceAction(new Resource("testResource", ResourceType.DATASOURCE), Action.WRITE));
	...
	public void testAddBadPermission() {
		...
		List<ResourceAction> permsToAdd = ImmutableList
				.of(new ResourceAction(new Resource("??????????", ResourceType.DATASOURCE), Action.WRITE));
