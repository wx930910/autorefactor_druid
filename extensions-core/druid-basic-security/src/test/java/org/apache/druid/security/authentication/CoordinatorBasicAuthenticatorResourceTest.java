...
public class CoordinatorBasicAuthenticatorResourceTest {
	...
	public void setUp() {
		...
		AuthenticatorMapper authenticatorMapper = new AuthenticatorMapper(ImmutableMap.of(AUTHENTICATOR_NAME,
				new BasicHTTPAuthenticator(null, AUTHENTICATOR_NAME, null, new DefaultPasswordProvider("druid"),
						new DefaultPasswordProvider("druid"), null, null, null, false, null),
				AUTHENTICATOR_NAME2,
				new BasicHTTPAuthenticator(null, AUTHENTICATOR_NAME2, null, new DefaultPasswordProvider("druid"),
						new DefaultPasswordProvider("druid"), null, null, null, false, null),
				AUTHENTICATOR_NAME_LDAP,
				new BasicHTTPAuthenticator(null, AUTHENTICATOR_NAME2, null, new DefaultPasswordProvider("druid"),
						new DefaultPasswordProvider("druid"), null, null, null, false, null)));

		storageUpdater = new CoordinatorBasicAuthenticatorMetadataStorageUpdater(authenticatorMapper, connector,
				tablesConfig, new BasicAuthCommonCacheConfig(null, null, null, null), objectMapper,
				NoopBasicAuthenticatorCacheNotifier.mockBasicAuthenticatorCacheNotifier1(), null);

		resource = new BasicAuthenticatorResource(
				new CoordinatorBasicAuthenticatorResourceHandler(storageUpdater, authenticatorMapper, objectMapper));
	...
	public void testInvalidAuthenticator() {
		...
		Assert.assertEquals(errorMapWithMsg("Basic authenticator with name [invalidName] does not exist."),
				response.getEntity());
	}
	...
	public void testGetAllUsers() {
		...
		Map<String, BasicAuthenticatorUser> cachedUserMap = BasicAuthUtils.deserializeAuthenticatorUserMap(objectMapper,
	...
	public void testGetAllUsersSeparateDatabaseTables() {
		...
		// Verify cached user map for AUTHENTICATOR_NAME authenticator is also getting
		// updated
		response = resource.getCachedSerializedUserMap(req, AUTHENTICATOR_NAME);
		Assert.assertEquals(200, response.getStatus());
		Assert.assertTrue(response.getEntity() instanceof byte[]);
	...
	public void testUserCredentials() {
		...
		response = resource.updateUserCredentials(req, AUTHENTICATOR_NAME, "druid",
